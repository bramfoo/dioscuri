...
public class Video extends ModuleVideo
{
    ...
    public boolean reset()
    {
        ...
        // Set monitor type to 'not CGA/MDA'; CMOS IBM_EQUIPMENT byte [0x14] bits 4,5
    ...
    public void update()
    {
        ...
        // Skip screen update if the vertical retrace is in progress (using 72 Hz vertical frequency)
        // FIXME: connection to CPU will be needed for this - and will need to be fairly accurate! 
        ...
        if (videocard.graphicsController.alphaNumDisable != 0)
        {
            ...
            switch (videocard.graphicsController.shift256Reg)
            {
                                ...
                                if ((videocard.getTileUpdate(xti, yti)))
                                {
                                    for (r = 0; r < VideoCard.Y_TILESIZE; r++)
                                    {
                                        ...
                                        for (c = 0; c < VideoCard.X_TILESIZE; c++)
                                        {
                                            ...
                                            attribute = (byte) ((((videocard.vgaMemory[plane0 + byteOffset] >> bitNumber) & 0x01) << 0)
                                                    | (((videocard.vgaMemory[plane1 + byteOffset] >> bitNumber) & 0x01) << 1)
                                                    | (((videocard.vgaMemory[plane2 + byteOffset] >> bitNumber) & 0x01) << 2) 
                                                    | (((videocard.vgaMemory[plane3 + byteOffset] >> bitNumber) & 0x01) << 3));

                                            attribute &= videocard.attributeController.colourPlaneEnable;
                                            // undocumented feature ???: colours 0..7 high intensity, colours 8..15 blinking
                                            ...
                                            if (videocard.attributeController.modeControlReg.paletteBitsSelect != 0)
                                            {
                                                ...
                                                // use 4 higher bits from colour select register
                                            ...
                                            else
                                            {
                                                ...
                                                // use 2 higher bits from colour select register
                ...
                case 1: // output the data in a CGA-compatible 320x200 4 colour graphics
                    // mode. (modes 4 & 5)
                ...
                case 3: // (Bochs)fixme: is this really the same ???
                    ...
                    if (videocard.sequencer.chainFourEnable != 0)
                    {
                        ...
                        if (videocard.miscOutputRegister.lowHighPage != 1)
                            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " update: select_high_bank != 1");
        ...
        else
        {
            ...
            int fullStartAddress = 2 * ((videocard.crtControllerRegister.regArray[0x0C] << 8) 
                    + videocard.crtControllerRegister.regArray[0x0D]); // Upper left character of screen
            ...
            // Check 8/9 pixel character width, adjust horizontal panning accordingly (??)
            ...
            // Check character height to update; if this is some silly value (i.e. not visible), ignore update
            if (maxScanLine == 0)
            {
