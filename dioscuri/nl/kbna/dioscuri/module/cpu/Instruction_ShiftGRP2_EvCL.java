 ...
 * Performs the selected instruction (indicated by bits 5, 4, 3 of the ModR/M byte) using CL.<BR>
...
public class Instruction_ShiftGRP2_EvCL implements Instruction
{
    ...
    public void execute() throws CPUInstructionException
    {
        ...
        // Get CL byte for number of shifts and limit it to 5 bits (only 0 - 31 shifts allowed)
        ...
        switch ((addressByte & 0x38) >> 3)
        {
            ...
            case 2: // RCL - Rotate bits + CF CL positions left; Flags affected: CF, OF
                // Execute rotate on reg or mem. Determine this from mm bits of addressbyte
                if (((addressByte >> 6) & 0x03) == 3)
                {
                    // RCL on register
            ...
            case 3: // RCR - Rotate bits + CF CL positions right; Flags affected: CF, OF
                // Execute rotate on reg or mem. Determine this from mm bits of addressbyte
                if (((addressByte >> 6) & 0x03) == 3)
                {
                    // RCR on register
            ...
            case 4: // SHL/SAL; flags affected: CF, OF, SF, ZF, PF, AF (undefined)
                ...
                if (bitShift > 0)
                {
                    ...
                    {
                        // "Officialy" (Intel docs) OF is only set if 1-bit shift occurred; in hardware, it always sets the flag
        //                if (bitShift == 1)
        //                {
        ...
        //                }
            ...
            case 5:  // SHR; unsigned. flags affected: CF, OF, SF, ZF, PF, AF (undefined)
                ...
                if (bitShift > 0)
                {
                    cpu.flags[CPU.REGISTER_FLAGS_OF] = false;   // According to Bochs, is always reset except when shift = 1 (handled below)
            ...
            case 7: // SAR; flags affected: CF, OF, SF, ZF, PF, AF (undefined)
                    ...
                    if (bitShift > 0)
                    {
                        ...
                        // If bitShift is >= 16, the result will be the MSB in all bit positions
                        if (bitShift >= 16)
                        {
                        ...
                        // bitShift < 16, so perform SHR and depending on MSB OR with (0xFFFF << 16 - bitShift) 
                        else
                        {
                            // Convert bytes to long and shift
                            shiftResult = (((((long)sourceValue[CPU.REGISTER_GENERAL_HIGH]) << 8) & 0xFF00) + 
                        ...
                        // Set ZF
                        cpu.flags[CPU.REGISTER_FLAGS_ZF] = sourceValue[CPU.REGISTER_GENERAL_HIGH] == 0 && sourceValue[CPU.REGISTER_GENERAL_LOW] == 0 ? true
                                : false;
                        // Set SF on particular byte of sourceValue (set when MSB is 1, occurs when destReg >= 0x80)
                        cpu.flags[CPU.REGISTER_FLAGS_SF] = sourceValue[CPU.REGISTER_GENERAL_HIGH] < 0 ? true : false;
                        // Set PF on particular byte of sourceValue
