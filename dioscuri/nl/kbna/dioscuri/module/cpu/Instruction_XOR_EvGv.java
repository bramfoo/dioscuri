...
package nl.kbna.dioscuri.module.cpu;
	...
	/**
	 * Intel opcode 31<BR>
	 * Logical word-sized XOR of memory/register (destination) and register (source).<BR>
...
public class Instruction_XOR_EvGv implements Instruction {
	...
	public void execute()
	{
        ...
        if (((addressByte >> 6) & 0x03) == 3)
		{
            ...
            else
            {
                ...
                cpu.flags[CPU.REGISTER_FLAGS_ZF] = destinationRegister[CPU.REGISTER_GENERAL_HIGH] == 0x00
                        ...
                        && eDestinationRegister[CPU.REGISTER_GENERAL_LOW] == 0x00 ? true : false;
                // Test SF on particular byte of destinationRegister (set when MSB is 1, occurs when destReg >= 0x80)
		...
		else
		{
			...
			cpu.setWordInMemorySegment(addressByte, memoryReferenceLocation, logicalXORResult);

            // Test ZF on result
            cpu.flags[CPU.REGISTER_FLAGS_ZF] = logicalXORResult[CPU.REGISTER_GENERAL_HIGH] == 0 && logicalXORResult[CPU.REGISTER_GENERAL_LOW] == 0  ? true : false;
            // Test SF on result (set when MSB is 1, occurs when result >= 0x80)
            cpu.flags[CPU.REGISTER_FLAGS_SF] = logicalXORResult[CPU.REGISTER_GENERAL_HIGH] < 0 ? true : false;
            ...
            if (cpu.doubleWord) // Dealing with extra registers
            {
                ...
                // Re-adjust flags
                cpu.flags[CPU.REGISTER_FLAGS_ZF] = cpu.flags[CPU.REGISTER_FLAGS_ZF] && logicalXORResult[CPU.REGISTER_GENERAL_HIGH] == 0 && logicalXORResult[CPU.REGISTER_GENERAL_LOW] == 0  ? true : false;
                cpu.flags[CPU.REGISTER_FLAGS_SF] = logicalXORResult[CPU.REGISTER_GENERAL_HIGH] < 0 ? true : false;
            }

            // Set PF on lower byte of result
			cpu.flags[CPU.REGISTER_FLAGS_PF] = Util.checkParityOfByte(logicalXORResult[CPU.REGISTER_GENERAL_LOW]);
