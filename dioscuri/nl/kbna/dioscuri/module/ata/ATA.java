 ...
 * ******************************************** 
 * general.type                 : ata
 * general.name                 : ATA Controller / ATA-1 to ATAT-3 
 * general.architecture         : Von Neumann 
 * general.description          : Implements a standard Integrated Drive Electronic (IDE) controller / parallel ATA 
 * general.creator              : Tessella Support Services, Koninklijke Bibliotheek, Nationaal Archief of the Netherlands 
 * general.version              : 1.0
 * general.keywords             : IDE, ATA, storage, disk, controller
 * general.relations            : motherboard 
 * general.yearOfIntroduction   : 1994 
 * general.yearOfEnding         : 1999 
 * general.ancestor             : - 
 * general.successor            : ATA-2 (EIDE)
 * 
 * Notes: none
 * 
 */
public class ATA extends ModuleATA
{
    ...
    public ATA(Emulator owner)
    {
        ...
        {
            channels[i] = new ATAChannel(this, ATAConstants.DEFAULT_IO_ADDR_1[i], ATAConstants.DEFAULT_IO_ADDR_2[i]);
        }

        curChannelIndex = 0;

        logger.log(Level.INFO, MODULE_TYPE.toString() + " " + MODULE_TYPE.getName() + " -> Module created successfully.");
    ...
    public boolean reset()
    {
        ...
        /*
         * // Register I/O ports 01F0-01F7 - Primary IDE Controller in I/O address space // motherboard.setIOPort(IDEConstants.PORT_IDE_DATA, this);
         * motherboard.setIOPort(IDEConstants.PORT_IDE_ERROR_WPC, this); motherboard.setIOPort(IDEConstants.PORT_IDE_SECTOR_COUNT, this);
         * motherboard.setIOPort(IDEConstants.PORT_IDE_SECTOR_NUMBER, this); motherboard.setIOPort(IDEConstants.PORT_IDE_CYLINDER_LOW, this);
         * motherboard.setIOPort(IDEConstants.PORT_IDE_CYLINDER_HIGH, this); motherboard.setIOPort(IDEConstants.PORT_IDE_DRIVE_HEAD, this);
         * motherboard.setIOPort(IDEConstants.PORT_IDE_STATUS_CMD, this); motherboard.setIOPort(IDEConstants.PORT_IDE_ALT_STATUS_DEVICE, this);
    ...
    {
        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  get data TO BE IMPLEMENTED");
        return null;
    }
    ...
    {
        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + " set data TO BE IMPLEMENTED");
        return false;
    }
    ...
    public String getDump()
    {
        ...
        for (int i = 0; i < channels.length; i++)
        {
            ...
            {
                if (drives[j] != null && drives[j].driveType != ATADriveType.NONE)
                {

                    dump += "Drive " + j + ": type '" + drives[j].getDriveType() + "'";
    ...
    public void update()
    {
        for (int channelIndex = 0; channelIndex < ATAConstants.MAX_NUMBER_IDE_CHANNELS; channelIndex++)
        {
            ...
            for (int deviceIndex = 0; deviceIndex < 2; deviceIndex++)
            {
                ...
                if (channels[channelIndex].getDrives()[deviceIndex].getIoLightCounter() > 0)
                {

                    int newIoLightCounter = channels[channelIndex].getDrives()[deviceIndex].getIoLightCounter();
                    newIoLightCounter--;

                    channels[channelIndex].getDrives()[deviceIndex].setIoLightCounter(newIoLightCounter);
     ...
     */
    public byte getIOPortByte(int originalPortAddress) throws ModuleException, ModuleUnknownPort, ModuleWriteOnlyPortException
    {
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  IN command (byte) to port " + Integer.toHexString(originalPortAddress).toUpperCase() + " received");

        return read(originalPortAddress, 1)[0];
    }

    /**
     * Set a byte in I/O address space at given port TODO: BOCHS code writes to both drives of a channel - this seems wrong but kept in this code to adi
     ...
     * @throws ModuleException, ModuleWriteOnlyPortException
     */
    public void setIOPortByte(int originalAddress, byte data) throws ModuleException, ModuleUnknownPort
    {
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  OUT command (byte: " + Integer.toHexString(data).toUpperCase() + ") to port "
                + Integer.toHexString(originalAddress).toUpperCase() + " received.");
        byte[] dataArray = {data};
    ...
    {
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  IN command (double word) to port " + Integer.toHexString(portAddress).toUpperCase() + " received");
    ...
    {
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  OUT command (double word) to port " + Integer.toHexString(portAddress).toUpperCase() + " received.");
     ...
     * @param imageFilePath
     */
    public void initConfig(int theIdeChannel, boolean isMaster, boolean isHardDisk, boolean isWriteProtected, int numCylinders, int numHeads,
            int numSectorsPerTrack, ATATranslationType translationType, String imageFilePath)
    {
        ...
        drive.setDiskCapacity(numCylinders, numHeads, numSectorsPerTrack, sectorsPerBlock);
    ...
    public void setCmosSettings(int[] bootDrives, boolean floppySigCheckDisabled)
    {
        ...
        rtc.setCMOSRegister(0x12, (byte) (0x00)); // start out with: no drive 0, no drive 1
        ...
        if (channels[theChannel].getDrives()[0].getDriveType() == ATADriveType.HARD_DISK)
        {
            ...
            rtc.setCMOSRegister(0x12, (byte) ((rtc.getCMOSRegister(0x12) & 0x0f) | 0xf0));

            rtc.setCMOSRegister(0x19, (byte) 47); // user definable type

            // AMI BIOS: 1st hard disk #cyl low byte
            rtc.setCMOSRegister(0x1b, (byte) (channels[theChannel].getSelectedDrive().getTotalNumCylinders() & 0x00ff));

            // AMI BIOS: 1st hard disk #cyl high byte
            rtc.setCMOSRegister(0x1c, (byte) ((channels[theChannel].getSelectedDrive().getTotalNumCylinders() & 0xff00) >> 8));

            // AMI BIOS: 1st hard disk #heads
            rtc.setCMOSRegister(0x1d, (byte) (channels[theChannel].getSelectedDrive().getTotalNumHeads()));
            ...
            boolean condition = channels[theChannel].getSelectedDrive().getTotalNumHeads() > 8;
            int intCondition = 0;
            ...
            rtc.setCMOSRegister(0x23, (byte) channels[theChannel].getSelectedDrive().getTotalNumSectors());
        ...
        if (channels[theChannel].getDrives()[1].getDriveType() == ATADriveType.HARD_DISK)
        {
            ...
            rtc.setCMOSRegister(0x12, (byte) ((rtc.getCMOSRegister(0x12) & 0xf0) | 0x0f));
            rtc.setCMOSRegister(0x1a, (byte) 47); // user definable type

            // AMI BIOS: 2nd hard disk #cyl low byte
            rtc.setCMOSRegister(0x24, (byte) (channels[theChannel].getSelectedDrive().getTotalNumCylinders() & 0x00ff));

            // AMI BIOS: 2nd hard disk #cyl high byte
            rtc.setCMOSRegister(0x25, (byte) ((channels[theChannel].getSelectedDrive().getTotalNumCylinders() & 0xff00) >> 8));

            // AMI BIOS: 2nd hard disk #heads
            rtc.setCMOSRegister(0x26, (byte) (channels[theChannel].getSelectedDrive().getTotalNumHeads()));
            ...
            rtc.setCMOSRegister(0x29, (byte) ((channels[theChannel].getSelectedDrive().getTotalNumHeads() > 8) ? 0x80 : 0x00));
            ...
            rtc.setCMOSRegister(0x2c, (byte) channels[theChannel].getSelectedDrive().getTotalNumSectors());
        }
        ...
        for (int channelIndex = 0; channelIndex < channels.length; channelIndex++)
        {
            ...
            for (int driveIndex = 0; driveIndex < channel.getDrives().length; driveIndex++)
            {
                ...
                if (curDrive.getDriveType() == ATADriveType.HARD_DISK)
                {
                    ...
                    ATATranslationType translationType = curDrive.getTranslationType();
                    ...
                    {
                        rtc.setCMOSRegister(reg, (byte) (rtc.getCMOSRegister(reg) | (0 << bitshift)));

                    }
                    else if (translationType == ATATranslationType.LBA)
                    {

                        rtc.setCMOSRegister(reg, (byte) (rtc.getCMOSRegister(reg) | (1 << bitshift)));

                    }
                    else if (translationType == ATATranslationType.LARGE)
                    {

                        rtc.setCMOSRegister(reg, (byte) (rtc.getCMOSRegister(reg) | (2 << bitshift)));

                    }
                    else if (translationType == ATATranslationType.RECHS)
                    {
                        rtc.setCMOSRegister(reg, (byte) (rtc.getCMOSRegister(reg) | (3 << bitshift)));
                    }
                }
            }
        ...
        rtc.setCMOSRegister(0x38, (byte) (intFloppySigCheckDisabled | (bootDrives[2] << 4)));
    ...
    private boolean loadDiskImage(String imageFilePath, ATADrive drive)
    {
        ...
        catch (StorageDeviceException sde)
        {
            ...
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Error: " + sde.getMessage());
            return false;
        }
    }
     ...
     * @throws ModuleException, ModuleWriteOnlyPortException
     */
    private byte[] read(int originalPortAddress, int ioLength) throws ModuleException, ModuleUnknownPort, ModuleWriteOnlyPortException
    {

//        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Read ide at instruction " + cpu.getCurrentInstructionNumber());
        ...
        for (channelIndex = 0; channelIndex < ATAConstants.MAX_NUMBER_IDE_CHANNELS; channelIndex++)
        {
            ...
            if ((originalPortAddress & 0xfff8) == channels[channelIndex].getIoAddress1())
            {
                channelPort = originalPortAddress - channels[channelIndex].getIoAddress1();
                break;

            }
            else if ((originalPortAddress & 0xfff8) == channels[channelIndex].getIoAddress2())
            {
                channelPort = originalPortAddress - channels[channelIndex].getIoAddress2() + 0x10;
                break;
            }
        }
        ...
        if (channelIndex == ATAConstants.MAX_NUMBER_IDE_CHANNELS)
        {
            ...
            if ((originalPortAddress < 0x03f6) || (originalPortAddress > 0x03f7))
            {

                logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  read: unable to find ATA channel, ioport=0x%04x "
        ...
        switch (channelPort)
        {
            ...
            case 0x01:
                ...
                channels[this.curChannelIndex].getSelectedController().getStatus().setErr(0);
                if (channels[this.curChannelIndex].isAnyDrivePresent())
                {
                    value[0] = (byte) channels[this.curChannelIndex].getSelectedController().getErrorRegister();
                }
                else
                {
                    value[0] = 0;
            ...
            case 0x02:
                ...
                if (channels[this.curChannelIndex].isAnyDrivePresent())
                {
                    value[0] = (byte) channels[this.curChannelIndex].getSelectedDrive().getSectorCount();

                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Read sector count returned value " + value[0]);
            ...
            case 0x03:
                ...
                if (channels[this.curChannelIndex].isAnyDrivePresent())
                {
                    value[0] = (byte) channels[this.curChannelIndex].getSelectedDrive().getCurrentSector();
                }
                else
                {
                    value[0] = 0;
            ...
            case 0x04:
                ...
                if (channels[this.curChannelIndex].isAnyDrivePresent())
                {
                    value[0] = (byte) (channels[this.curChannelIndex].getSelectedDrive().getCurrentCylinder() & 0x00ff);
                }
                else
                {
                    value[0] = 0;
            ...
            case 0x05:
                ...
                if (channels[this.curChannelIndex].isAnyDrivePresent())
                {
                    value[0] = (byte) (channels[this.curChannelIndex].getSelectedDrive().getCurrentCylinder() >> 8);
            ...
            case 0x06:
                ...
                int intlbaMode = 0;
                if (channels[this.curChannelIndex].getSelectedController().getLbaMode() > 0)
                {
                    intlbaMode = 1;
                }

                value[0] = (byte) ((1 << 7) | (intlbaMode << 6) | (1 << 5) | // 01b = 512 sector size
                        (channels[this.curChannelIndex].getSelectedDriveIndex() << 4) | (channels[this.curChannelIndex].getSelectedDrive().getCurrentHead() << 0));
            ...
            case 0x16:
                ...
                else
                {
                    ...
                    value[0] = (byte) ((channels[this.curChannelIndex].getSelectedController().getStatus().getBusy() << 7)
                            | (channels[this.curChannelIndex].getSelectedController().getStatus().getDriveReady() << 6)
                            | (channels[this.curChannelIndex].getSelectedController().getStatus().getWriteFault() << 5)
                            | (channels[this.curChannelIndex].getSelectedController().getStatus().getSeekComplete() << 4)
                            | (channels[this.curChannelIndex].getSelectedController().getStatus().getDrq() << 3)
                            | (channels[this.curChannelIndex].getSelectedController().getStatus().getCorrectedData() << 2)
                            | (channels[this.curChannelIndex].getSelectedController().getStatus().getIndexPulse() << 1) | (channels[this.curChannelIndex]
                            .getSelectedController().getStatus().getErr()));

                    channels[this.curChannelIndex].getSelectedController().getStatus().incrementIndexPulseCount();
                    channels[this.curChannelIndex].getSelectedController().getStatus().setIndexPulse(0);

                    if (channels[this.curChannelIndex].getSelectedController().getStatus().getIndexPulseCount() >= ATAConstants.INDEX_PULSE_CYCLE)
                    {
                        channels[this.curChannelIndex].getSelectedController().getStatus().setIndexPulse(1);
                        channels[this.curChannelIndex].getSelectedController().getStatus().setIndexPulseCount(0);
                    }
            ...
            default:
                ...
                logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Port [0x" + Integer.toHexString(originalPortAddress).toUpperCase()
                        + "] returned default value 0xFF");
                return value;
        }

        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Read from Port [0x" + Integer.toHexString(originalPortAddress).toUpperCase() + "] returned value " + value[0]);
    ...
    private byte[] readSectors(int originalAddress, int ioLength)
    {
        ...
        if (channels[this.curChannelIndex].getSelectedController().getBufferIndex() >= channels[this.curChannelIndex].getSelectedController().getBufferSize())
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO read buffer_index >= 512, for address " + originalAddress + ".");
            return value;
        }
        ...
        if (this.bulkIOQuantumsRequested > 0 && ATAConstants.SUPPORT_REPEAT_SPEEDUPS)
        {
            ...
            int quantumsMax = (channels[this.curChannelIndex].getSelectedController().getBufferSize() - getSelectedDriveController().getBufferIndex())
                    / ioLength;

            if (quantumsMax == 0)
            {
                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO read not enough space for read for address " + originalAddress + ".");
                ;
            ...
            int newBufferIndex = channels[this.curChannelIndex].getSelectedController().getBufferIndex() + transferLen;
            channels[this.curChannelIndex].getSelectedController().setBufferIndex(newBufferIndex);
        ...
        else
        {
            ...
            if (ioLength >= 2)
            {
                value[0] = channels[this.curChannelIndex].getSelectedController().getBuffer()[channels[this.curChannelIndex].getSelectedController()
                        .getBufferIndex()];
                value[1] = channels[this.curChannelIndex].getSelectedController().getBuffer()[channels[this.curChannelIndex].getSelectedController()
                        .getBufferIndex() + 1];
            }
            ...
            if (ioLength == 4)
            {
                ...
                value[2] = channels[this.curChannelIndex].getSelectedController().getBuffer()[channels[this.curChannelIndex].getSelectedController()
                        .getBufferIndex() + 2];

                value[3] = channels[this.curChannelIndex].getSelectedController().getBuffer()[channels[this.curChannelIndex].getSelectedController()
                        .getBufferIndex() + 3];

                long full32bitintvalue = (((long) value[0]) & 0xFF) + (((long) (value[1]) & 0xFF) << 8) + (((long) (value[2]) & 0xFF) << 16)
                        + (((long) (value[3]) & 0xFF) << 24);

                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  IO read returns values for index "
                        + channels[this.curChannelIndex].getSelectedController().getBufferIndex() + " " + value[0] + ", " + value[1] + ", " + value[2]
                        + " and " + value[3] + " = " + full32bitintvalue);

            }

            int newBufferIndex = channels[this.curChannelIndex].getSelectedController().getBufferIndex() + ioLength;
            channels[this.curChannelIndex].getSelectedController().setBufferIndex(newBufferIndex);
        ...
        if (channels[this.curChannelIndex].getSelectedController().getBufferIndex() >= channels[this.curChannelIndex].getSelectedController().getBufferSize())
        {
            ...
            //        
            if ((byte) channels[this.curChannelIndex].getSelectedController().getCurrentCommand() == (byte) 0xC4)
            {

                if (channels[this.curChannelIndex].getSelectedDrive().getSectorCount() > channels[this.curChannelIndex].getSelectedController()
                        .getMultipleSectors())
                {
                    channels[this.curChannelIndex].getSelectedController().setBufferSize(
                            channels[this.curChannelIndex].getSelectedController().getMultipleSectors() * 512);
                }
                else
                {
                    channels[this.curChannelIndex].getSelectedController().setBufferSize(
                            channels[this.curChannelIndex].getSelectedDrive().getSectorCount() * 512);
                }
            }
            ...
            { /* read next one into controller buffer */
                ...
                if (ideReadData(curChannelIndex, getSelectedDriveController().getBuffer(), getSelectedDriveController().getBufferSize()))
                {
     ...
     */
    private boolean write(int originalAddress, byte[] data, int ioLength) throws ModuleException, ModuleUnknownPort
    {

//        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Write ide at instruction " + cpu.getCurrentInstructionNumber());
        ...
        Integer logicalSector = new Integer(0); // Make Integer object so can pass by reference
        int ret = 0;
        ...
        for (channelIndex = 0; channelIndex < ATAConstants.MAX_NUMBER_IDE_CHANNELS; channelIndex++)
        {
            ...
            if ((originalAddress & 0xfff8) == channels[channelIndex].getIoAddress1())
            {
                channelPort = originalAddress - channels[channelIndex].getIoAddress1();
                break;

            }
            else if ((originalAddress & 0xfff8) == channels[channelIndex].getIoAddress2())
            {
                channelPort = originalAddress - channels[channelIndex].getIoAddress2() + 0x10;
                break;
            }
        }
        ...
        if (channelIndex == ATAConstants.MAX_NUMBER_IDE_CHANNELS)
        {
            ...
            {
                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  write: unable to find ATA channel, ioport " + originalAddress + ".");
        ...
        switch (channelPort)
        {
            ...
            case 0x04: // hard disk cylinder low 0x1f4
                ...
                {
                    int newValue0 = (int) ((getSelectedChannel().getDrives()[0].getCurrentCylinder() & 0xff00) | intData[0]);
                    getSelectedChannel().getDrives()[0].setCurrentCylinder(newValue0);
        
                    int newValue1 = (int) ((getSelectedChannel().getDrives()[1].getCurrentCylinder() & 0xff00) | intData[0]);
            ...
            case 0x05: // hard disk cylinder high 0x1f5
                ...
                {
                    int newValue0 = (int) ((intData[0] << 8) | (getSelectedChannel().getDrives()[0].getCurrentCylinder() & 0xff));
                    getSelectedChannel().getDrives()[0].setCurrentCylinder(newValue0);
    
                    int newValue1 = (int) ((intData[0] << 8) | (getSelectedChannel().getDrives()[1].getCurrentCylinder() & 0xff));
                    getSelectedChannel().getDrives()[1].setCurrentCylinder(newValue1);
                }
                break;
            ...
            case 0x07: // hard disk command 0x1f7
                ...
                this.setHardDiskCommand(originalAddress, intData, logicalSector, ret);
                break;

            case 0x16: // 0x3f6
                // PORT_IDE_ALT_STATUS_DEVICE
                this.setPortIdeAltStatusDevice(originalAddress, prevControlReset, intData);
                break;
     ...
     * @param ioLength
     */
    private void writeSectors(int originalAddress, int[] data, int channel, int ioLength)
    {

        if (getSelectedDriveController().getBufferIndex() >= 512)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO write to address " + originalAddress + " buffer index >= 512.");
        }
        ...
        if (bulkIOQuantumsRequested > 0 && ATAConstants.SUPPORT_REPEAT_SPEEDUPS)
        {
            ...
            quantumsMax = (512 - getSelectedDriveController().getBufferIndex()) / ioLength;

            if (quantumsMax == 0)
            {
                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO write to address " + originalAddress + " not enough space for write.");
            }
        ...
        else
        {
            ...
            if (ioLength >= 2)
            {
                getSelectedDriveController().setBuffer(getSelectedDriveController().getBufferIndex() + 1, (byte) data[1]);
                getSelectedDriveController().setBuffer(getSelectedDriveController().getBufferIndex(), (byte) data[0]);
            }

            if (ioLength == 4)
            {
                getSelectedDriveController().setBuffer(getSelectedDriveController().getBufferIndex() + 3, (byte) data[3]);
                getSelectedDriveController().setBuffer(getSelectedDriveController().getBufferIndex() + 2, (byte) data[2]);
            }
        ...
        if (getSelectedDriveController().getBufferIndex() >= getSelectedDriveController().getBufferSize())
        {

            if (ideWriteData(curChannelIndex, getSelectedDriveController().getBuffer(), getSelectedDriveController().getBufferSize()))
            {

                if (getSelectedDriveController().getCurrentCommand() == (byte) 0xC5)
                {
                    if (getSelectedDrive().getSectorCount() > getSelectedDriveController().getMultipleSectors())
                    {
                        getSelectedDriveController().setBufferSize(getSelectedDriveController().getMultipleSectors() * 512);

                    }
                    else
                    {
                        getSelectedDriveController().setBufferSize(getSelectedDrive().getSectorCount() * 512);
                    }
                }
                getSelectedDriveController().setBufferIndex(0);

                /*
                 * When the write is complete, controller clears the DRQ bit and sets the BSY bit. If at least one more sector is to be written, controller sets
                 * DRQ bit, clears BSY bit, and issues IRQ
                 */
    ...
    private byte[] executeGetCommand(int originalAddress, int ioLength)
    {
        ...
        if (getSelectedDriveController().getStatus().getDrq() == 0)
        {
            ...
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO read(" + originalAddress + ") with drq == 0: last command was " + command.toString() + ", address "
                    + command.getAddress() + " IO Length " + ioLength);
        ...
        if (command != null)
        {
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  IO read(" + originalAddress + ") current command is " + command.toString() + ",address "
                    + command.getAddress() + " IO Length " + ioLength);
        }
        ...
        if (command == null)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  read cmd " + currentCommand + " not recognised");
        ...
        else if (command == ATACommand.READ_SECTORS_WITH_RETRY // case 0x20: // READ SECTORS, with retries
                || command == ATACommand.READ_SECTORS_WITHOUT_RETRY // case 0x21: // READ SECTORS, without retries
                || command == ATACommand.READ_MULTIPLE) // case 0xC4: // READ MULTIPLE SECTORS
        {

            value = this.readSectors(originalAddress, ioLength);

        }
        else if (command == ATACommand.IDENTIFY_DRIVE // TODO: this is the code from bochs to check
                || command == ATACommand.PACKET_A1)
        {
            // IDENTIFY DEVICE
            // case 0xa1:

            channels[this.curChannelIndex].getSelectedController().getStatus().setBusy(0);
            channels[this.curChannelIndex].getSelectedController().getStatus().setDriveReady(1);
            channels[this.curChannelIndex].getSelectedController().getStatus().setWriteFault(0);
            channels[this.curChannelIndex].getSelectedController().getStatus().setSeekComplete(1);
            channels[this.curChannelIndex].getSelectedController().getStatus().setCorrectedData(0);
            channels[this.curChannelIndex].getSelectedController().getStatus().setErr(0);

            int index = channels[this.curChannelIndex].getSelectedController().getBufferIndex();

            value[0] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index];
            index++;

            if (ioLength >= 2)
            {
                value[1] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index];

                logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  IO identify drive for index " + (index - 1) + " returns values " + value[0] + " and " + value[1]);
                index++;
            }
            if (ioLength == 4)
            {
                value[2] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index];
                value[3] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index + 1];
                index += 2;
            }

            channels[this.curChannelIndex].getSelectedController().setBufferIndex(index);

            if (channels[this.curChannelIndex].getSelectedController().getBufferIndex() >= 512)
            {

                channels[this.curChannelIndex].getSelectedController().getStatus().setDrq(0);
            }
        }
        // 0x0A0
        else if (command == ATACommand.PACKET_A0)
        {
            ...
            if (index >= channels[this.curChannelIndex].getSelectedController().getBufferSize())
            {
                if (index > channels[this.curChannelIndex].getSelectedController().getBufferSize())
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Read IO Packet A0 - index is greater than the bufferr size.");
                }
                ...
                switch (getSelectedDrive().getAtpi().getCommand())
                {
                    ...
                    case 0xbe: // read cd
                        ...
                        logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Read with no LOWLEVEL_CDROM.");
                        break;
            ...
            value[0] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index + increment];
            increment++;

            if (ioLength >= 2)
            {
                value[1] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index + increment];
                increment++;
            }
            if (ioLength == 4)
            {
                value[2] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index + increment];
                value[3] = channels[this.curChannelIndex].getSelectedController().getBuffer()[index + increment + 1];
                increment += 2;
            }

            channels[this.curChannelIndex].getSelectedController().setBufferIndex(index + increment);

            int newDrqIndex = channels[this.curChannelIndex].getSelectedController().getDrqIndex() + increment;
            channels[this.curChannelIndex].getSelectedController().setDrqIndex(newDrqIndex);

            if (channels[this.curChannelIndex].getSelectedController().getDrqIndex()
            >= Math.abs(channels[this.curChannelIndex].getSelectedDrive().getAtpi().getDrqBytes()))
            {

                channels[this.curChannelIndex].getSelectedController().getStatus().setDrq(0);
                channels[this.curChannelIndex].getSelectedController().setDrqIndex(0);

                int newTotalBytesRemaining = channels[this.curChannelIndex].getSelectedDrive().getAtpi().getTotalBytesRemaining()
                        - channels[this.curChannelIndex].getSelectedDrive().getAtpi().getDrqBytes();

                channels[this.curChannelIndex].getSelectedDrive().getAtpi().setTotalBytesRemaining(newTotalBytesRemaining);

                if (channels[this.curChannelIndex].getSelectedDrive().getAtpi().getTotalBytesRemaining() > 0)
                {

                    // one or more blocks remaining (works only for single block commands)
                    channels[this.curChannelIndex].getSelectedController().getInterruptReason().setIo(1);
                    channels[this.curChannelIndex].getSelectedController().getStatus().setBusy(0);
                    channels[this.curChannelIndex].getSelectedController().getStatus().setDrq(1);
                    channels[this.curChannelIndex].getSelectedController().getInterruptReason().setCd(0);

                    // set new byte count if last block
                    if (channels[this.curChannelIndex].getSelectedDrive().getAtpi().getTotalBytesRemaining() < channels[this.curChannelIndex]
                            .getSelectedController().getByteCount())
                    {

                        int totalBytesRemaining = channels[this.curChannelIndex].getSelectedDrive().getAtpi().getTotalBytesRemaining();
                        channels[this.curChannelIndex].getSelectedController().setByteCount(totalBytesRemaining);
                    }

                    int byteCount = channels[this.curChannelIndex].getSelectedController().getByteCount();
                    channels[this.curChannelIndex].getSelectedDrive().getAtpi().setDrqBytes(byteCount);
                ...
                else
                {
                    // all bytes read
                    channels[this.curChannelIndex].getSelectedController().getInterruptReason().setIo(1);
                    channels[this.curChannelIndex].getSelectedController().getInterruptReason().setCd(1);
                    channels[this.curChannelIndex].getSelectedController().getStatus().setDriveReady(1);
                    channels[this.curChannelIndex].getSelectedController().getInterruptReason().setRel(0);
                    channels[this.curChannelIndex].getSelectedController().getStatus().setBusy(0);
                    channels[this.curChannelIndex].getSelectedController().getStatus().setDrq(0);
                    channels[this.curChannelIndex].getSelectedController().getStatus().setErr(0);
        ...
        {
            logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  IO read current command " + command.getAddress() + " (" + command.getName() + ") not supported.");
        }
    ...
    {
        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  aborting: channel " + channel + "data " + value + ".");
    ...
    private void raiseInterrupt(int channel)
    {
        ...
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  raise interrupt called, disable_irq = " + getSelectedDriveController().isDisableIrq() + ".");

        if (!getSelectedDriveController().isDisableIrq())
        {
            logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  raising interrupt.");

        }
        else
        {
            logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  not raising interrupt.");
        }
        ...
        if (!getSelectedDriveController().isDisableIrq())
        {
            ...
            logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Raising interrupt " + irq + " on channel " + channel + ".");
    ...
    private void setPortIdeDriveHead(int originalAddress, int[] data)
    {
        ...
        {
            logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  IO write to address " + originalAddress + ", data " + data + " invalid as not 1x1xxxxxb.");
        }
        ...
        if (getSelectedChannel().getSelectedController().getLbaMode() == 0 && ((data[0] >> 6) & 1) == 1)
        {
            logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Enabling LBA mode.");
        }

        int newLdaValue = (int) ((data[0] >> 6) & 1);
        getSelectedChannel().getDrives()[0].getControl().setLbaMode(newLdaValue);
        getSelectedChannel().getDrives()[1].getControl().setLbaMode(newLdaValue);

        if (getSelectedDrive().getDriveType() == ATADriveType.NONE)
        {
            logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  ATA device for channel " + curChannelIndex + " set to " + driveSelect + " which does not exist.");
        }
     ...
     * @param data
     */
    private void setPortIdeAltStatusDevice(int originalAddress, boolean prevControlReset, int[] data)
    {
        ...
        getSelectedChannel().getDrives()[0].getControl().setDisableIrq(disableIrqValue);
        getSelectedChannel().getDrives()[1].getControl().setDisableIrq(disableIrqValue);

        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Adapter control reg: reset controller set to " + getSelectedDriveController().isReset() + " for channel "
                + curChannelIndex + ".");

        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Adapter control reg: disable_irq(X) set to " + getSelectedDriveController().isDisableIrq() + " for channel "
                + curChannelIndex + ".");
        ...
        if (!prevControlReset && getSelectedDriveController().isReset())
        {
            ...
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Hard drive: RESET.");

            // (mch) Set BSY, drive not ready
            for (int driveId = 0; driveId < 2; driveId++)
            {

                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setBusy(1);
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setDriveReady(0);
                getSelectedChannel().getDrives()[driveId].getControl().setResetInProgress(1);

                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setWriteFault(0);
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setSeekComplete(1);
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setDrq(0);
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setCorrectedData(0);
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setErr(0);

                getSelectedChannel().getDrives()[driveId].getControl().setErrorRegister(0x01); // diagnostic code: no error

                getSelectedChannel().getDrives()[driveId].getControl().setCurrentCommand(0x00);
                getSelectedChannel().getDrives()[driveId].getControl().setBufferIndex(0);

                getSelectedChannel().getDrives()[driveId].getControl().setMultipleSectors(0);
                getSelectedChannel().getDrives()[driveId].getControl().setLbaMode(0);

                getSelectedChannel().getDrives()[driveId].getControl().setDisableIrq(false);

                // TODO as per BOCHS this code is inside a loop, but seems unnecessary
                pic.clearIRQ(getSelectedChannel().getIrqNumber());
            }

        } else if (getSelectedDriveController().getResetInProgress() > 0 && !getSelectedDriveController().isReset())
        {
            // Clear BSY and DRDY

            logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Reset complete for channel " + curChannelIndex);

            for (int driveId = 0; driveId < 2; driveId++)
            {
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setBusy(0);
                getSelectedChannel().getDrives()[driveId].getControl().getStatus().setDriveReady(1);
                getSelectedChannel().getDrives()[driveId].getControl().setResetInProgress(0);
    ...
    /**
     * Execute command of IDC Note: assumed is that all bytes of the command are fetched. After execution of the command, the IDC will automatically enter the
     * result or idle phase.
     * 
     * @param originalAddress
     * @param data
     * @param ioLength
    ...
    private void executeSetCommand(int originalAddress, int[] data, int ioLength)
    {
        ...
        if (command == null)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  write cmd " + currentCommand + " not recognised");

            return;
        }

        else if (command == ATACommand.WRITE_SECTORS_WITH_RETRY || command == ATACommand.WRITE_MULTIPLE)
        {
        ...
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  write cmd " + command.getAddress() + " (" + command.getName() + ") not supported");
    ...
    private boolean ideReadData(int channel, byte[] buffer, int bufferSize)
    {
        ...
        do
        {
            if (!getSelectedDrive().calculateLogicalAddress(logicalSector))
            {
                ...
                logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  read sector reached invalid sector " + logicalSector + " aborting.");
                abortCommand(channel, channels[this.curChannelIndex].getSelectedController().getCurrentCommand());
                return false;
            }
            ...
            try
            {
                ...
                byte[] theData = getSelectedDrive().readData(getSelectedDriveController().getBuffer(), logicalSector * 512, 512);
            ...
            {
                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  logical sector was " + logicalSector + ".");
                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  could not read() hard drive image file at byte " + logicalSector * 512 + ".");
    ...
    private boolean ideWriteData(int channel, byte[] buffer, int bufferSize)
    {
        ...
        do
        {
            ...
            if (!getSelectedDrive().calculateLogicalAddress(logicalSector))
            {
                ...
                logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  IO write reached invalid sector " + logicalSector + ", aborting.");
                abortCommand(channel, getSelectedDriveController().getCurrentCommand());
                return false;
            }
            ...
            try
            {
                ...
                getSelectedDrive().writeData(getSelectedDriveController().getBuffer(), logicalSector * 512, 512);
            ...
            {
                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO write could not write()hard drive image file at byte " + logicalSector * 512 + ", aborting.");
            ...
            /*
             * update sector count, sector number, cylinder, drive, head, status if there are more sectors, read next one in...
             */
     ...
     * @param ioLength
     */
    private void setPacketA0(int originalAddress, int channel, int[] data, int ioLength)
    {

        if (getSelectedDriveController().getBufferIndex() >= ATAConstants.PACKET_SIZE)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO write to adress, " + originalAddress + " buffer_index >= PACKET_SIZE.");
            return;
        }
        ...
        if (getSelectedDriveController().getBufferIndex() >= ATAConstants.PACKET_SIZE)
        {
            ...
            switch (atapiCommand)
            {
                case (byte) 0x00: // test unit ready
                    ...
                    {
                        atapiCmdError(channel, SenseType.NOT_READY, AscType.MEDIUM_NOT_PRESENT, false);
                    }
                    raiseInterrupt(channel);
                ...
                case (byte) 0x03: // request sense
                {
                    ...
                    initSendAtapiCommand(channel, atapiCommand, 18, allocLength, false);

                    // sense data
                    getSelectedDriveController().setBuffer(0, (byte) (0x70 | (1 << 7)));
                    getSelectedDriveController().setBuffer(1, (byte) (0));
                    getSelectedDriveController().setBuffer(2, (byte) getSelectedDrive().getSenseInfo().getSenseKey().getValue());
                    getSelectedDriveController().setBuffer(3, (byte) getSelectedDrive().getSenseInfo().getInformation()[0]);
                    getSelectedDriveController().setBuffer(4, (byte) getSelectedDrive().getSenseInfo().getInformation()[1]);
                    getSelectedDriveController().setBuffer(5, (byte) getSelectedDrive().getSenseInfo().getInformation()[2]);
                    getSelectedDriveController().setBuffer(6, (byte) getSelectedDrive().getSenseInfo().getInformation()[3]);
                    getSelectedDriveController().setBuffer(7, (byte) (17 - 7)); // TODO: 17 -7 ???
                    getSelectedDriveController().setBuffer(8, (byte) getSelectedDrive().getSenseInfo().getSpecificInf()[0]);
                    getSelectedDriveController().setBuffer(9, (byte) getSelectedDrive().getSenseInfo().getSpecificInf()[1]);
                    getSelectedDriveController().setBuffer(10, (byte) getSelectedDrive().getSenseInfo().getSpecificInf()[2]);
                    getSelectedDriveController().setBuffer(11, (byte) getSelectedDrive().getSenseInfo().getSpecificInf()[3]);
                    getSelectedDriveController().setBuffer(12, (byte) getSelectedDrive().getSenseInfo().getAsc().getValue());
                    getSelectedDriveController().setBuffer(13, (byte) getSelectedDrive().getSenseInfo().getAscq());
                    getSelectedDriveController().setBuffer(14, (byte) getSelectedDrive().getSenseInfo().getFruc());
                    getSelectedDriveController().setBuffer(15, (byte) getSelectedDrive().getSenseInfo().getKeySpec()[0]);
                    getSelectedDriveController().setBuffer(16, (byte) getSelectedDrive().getSenseInfo().getKeySpec()[1]);
                    getSelectedDriveController().setBuffer(17, (byte) getSelectedDrive().getSenseInfo().getKeySpec()[2]);
                ...
                {
                    // bx_bool Immed = (BX_SELECTED_CONTROLLER(channel).buffer[1] >> 0) & 1; //BOCHS comment
                    ...
                    { // stop the disc
                        ...
                        logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Stop disc not implemented.");

                        atapiCmdNop(channel);
                        raiseInterrupt(channel);

                    }
                    else if (!(loEj > 0) && (start > 0))
                    { // start (spin up) the disc

                        if (ATAConstants.IS_LOW_LEVEL_CDROM)
                        {
                            // TODO: BX_SELECTED_DRIVE(channel).cdrom.cd->start_cdrom();
                        }

                        // TODO: ATAPI start disc not reading TOC
                        logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  ATAPI start disc not reading TOC.");
                        atapiCmdNop(channel);
                        raiseInterrupt(channel);
                ...
                case (byte) 0xbd: // mechanism status
                {
                    ...
                    int allocLength = 0;
                    // allocLength = read_16bit(BgetSelectedDriveController().getBuffer() + 8);
                    // TODO:check 8 is the offset
                    allocLength = read16bit(getSelectedDriveController().getBuffer(), 8);

                    if (allocLength == 0)
                    {
                        logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Not implemented: Zero allocation length to MECHANISM STATUS.");
                        return;
                    }

                    initSendAtapiCommand(channel, atapiCommand, 8, allocLength, false);

                    getSelectedDriveController().setBuffer(0, (byte) 0); // reserved for non changers
                    getSelectedDriveController().setBuffer(1, (byte) 0); // reserved for non changers

                    // TODO: seems to be set on dummy values
                    getSelectedDriveController().setBuffer(2, (byte) 0); // Current LBA
                    getSelectedDriveController().setBuffer(3, (byte) 0); // Current LBA
                    getSelectedDriveController().setBuffer(4, (byte) 0); // Current LBA

                    getSelectedDriveController().setBuffer(5, (byte) 1); // one slot

                    getSelectedDriveController().setBuffer(6, (byte) 0); // slot table length
                    getSelectedDriveController().setBuffer(7, (byte) 0); // slot table length
                ...
                case (byte) 0x5a: // mode sense
                {
                    ...
                    allocLength = read16bit(getSelectedDriveController().getBuffer(), 7);
                    ...
                    switch (pc)
                    {
                        case 0x0: // current values
                            switch (pageCode)
                            {
                                case 0x01: // error recovery
                                    ...
                                    initSendAtapiCommand(channel, atapiCommand, (getSelectedDrive().getCdRom().getErrorRecovery().length + 8), allocLength,
                                            false);

                                    // TODO: error recove is by ref initModeSenseSingle(channel, &BX_SELECTED_DRIVE(channel).cdrom.current.error_recovery,
                                    // sizeof(error_recovery_t));

                                    // Object[] array = getSelectedDrive().getCdRom().getErrorRecovery();
                                    // TODO:
                                    // initModeSenseSingle(channel, getSelectedDrive().getCdRom().getErrorRecovery(),
                                ...
                                case 0x2a: // CD-ROM capabilities & mech. status
                                    ...
                                    getSelectedDriveController().setBuffer(13, (byte) (3 << 5));
                                    getSelectedDriveController().setBuffer(14, (byte) (1 | // TODO: previously char
                                            (getSelectedDrive().getCdRom().isLocked() ? (1 << 1) : 0) | (1 << 3) | (1 << 5)));
                                    getSelectedDriveController().setBuffer(15, (byte) 0x00);
                                    getSelectedDriveController().setBuffer(16, (byte) ((706 >> 8) & 0xff));
                                    getSelectedDriveController().setBuffer(17, (byte) (706 & 0xff));
                                    getSelectedDriveController().setBuffer(18, (byte) 0);
                                    getSelectedDriveController().setBuffer(19, (byte) 2);
                                    getSelectedDriveController().setBuffer(20, (byte) ((512 >> 8) & 0xff));
                                    getSelectedDriveController().setBuffer(21, (byte) (512 & 0xff));
                                    getSelectedDriveController().setBuffer(22, (byte) ((706 >> 8) & 0xff));
                                    getSelectedDriveController().setBuffer(23, (byte) (706 & 0xff));
                                ...
                                case 0x3f: // all
                                    logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  cdrom: MODE SENSE (curr), page code " + pageCode + ", not implemented yet.");
                                    atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.INV_FIELD_IN_CMD_PACKET, true);
                                    raiseInterrupt(channel);
                                    break;

                                default:
                                    // not implemeted by this device
                                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  cdrom: MODE SENSE pc " + pc + ", page code, " + pageCode
                        ...
                        case 0x1: // changeable values
                            switch (pageCode)
                            {
                                ...
                                case 0x3f: // all
                                    ...
                                    logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  cdrom: MODE SENSE (chg), page code " + pageCode + ", not implemented yet.");
                                    atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.INV_FIELD_IN_CMD_PACKET, true);
                                    raiseInterrupt(channel);
                                    break;
                        ...
                        case 0x2: // default values
                            switch (pageCode)
                            {
                                case 0x2a: // CD-ROM capabilities & mech. status, copied from current values
                                    ...
                                    // getSelectedDriveController().setBuffer(14, (unsigned char) (1 | //TODO: check use char
                                    getSelectedDriveController().setBuffer(14,
                                            (byte) (1 | (getSelectedDrive().getCdRom().isLocked() ? (1 << 1) : 0) | (1 << 3) | (1 << 5)));
                                ...
                                case 0x3f: // all
                                    ...
                                    logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  cdrom: MODE SENSE (dflt), page code " + pageCode + ", not implemented.");
                        ...
                        case 0x3: // saved values not implemented
                            atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.SAVING_PARAMETERS_NOT_SUPPORTED, true);
                            raiseInterrupt(channel);
                            break;

                        default:
                            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  IO Write setPacketA0 pc value not recognised.");
                ...
                case (byte) 0x12: // inquiry
                {
                    ...
                    initSendAtapiCommand(channel, atapiCommand, 36, allocLength, false);

                    getSelectedDriveController().setBuffer(0, (byte) 0x05); // CD-ROM
                    getSelectedDriveController().setBuffer(1, (byte) 0x80); // Removable
                    getSelectedDriveController().setBuffer(2, (byte) 0x00); // ISO, ECMA, ANSI version
                    getSelectedDriveController().setBuffer(3, (byte) 0x21); // ATAPI-2, as specified
                    getSelectedDriveController().setBuffer(4, (byte) 31); // additional length (total 36) TODO: 0x is missing in BOCHS?
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        getSelectedDriveController().setBuffer(8 + i, (byte) vendorId[i]);
                    }
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        getSelectedDriveController().setBuffer(16 + i, (byte) productId[i]);
                    }
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        getSelectedDriveController().setBuffer(32 + i, (byte) revLevel[i]);
                    }
                ...
                case (byte) 0x25: // read cd-rom capacity
                {
                    ...
                    if (getSelectedDrive().getCdRom().isReady())
                    {
                        ...
                        getSelectedDriveController().setBuffer(0, (byte) ((capacity >> 24) & 0xff));
                        getSelectedDriveController().setBuffer(1, (byte) ((capacity >> 16) & 0xff));
                        getSelectedDriveController().setBuffer(2, (byte) ((capacity >> 8) & 0xff));
                        getSelectedDriveController().setBuffer(3, (byte) ((capacity >> 0) & 0xff));
                        getSelectedDriveController().setBuffer(4, (byte) ((2048 >> 24) & 0xff));
                        getSelectedDriveController().setBuffer(5, (byte) ((2048 >> 16) & 0xff));
                        getSelectedDriveController().setBuffer(6, (byte) ((2048 >> 8) & 0xff));
                        getSelectedDriveController().setBuffer(7, (byte) ((2048 >> 0) & 0xff));
                    ...
                    {
                        atapiCmdError(channel, SenseType.NOT_READY, AscType.MEDIUM_NOT_PRESENT, true);
                        raiseInterrupt(channel);
                    }
                }
                    break;
                case (byte) 0xbe: // read cd
                {
                    ...
                    {
                        int lba = 0;
                        // TODO int lba = read_32bit(getSelectedDriveController().getBuffer() + 2);
                        lba = read32bit(getSelectedDriveController().getBuffer(), 2);

                        int transferLength = getSelectedDriveController().getBuffer()[8] | (getSelectedDriveController().getBuffer()[7] << 8)
                        ...
                        switch (transferReq & 0xf8)
                        {
                            ...
                            case 0x10:
                                ...
                                initSendAtapiCommand(channel, atapiCommand, transferLength * getSelectedDriveController().getBufferSize(), transferLength
                                        * getSelectedDriveController().getBufferSize(), true);

                                getSelectedDrive().getCdRom().setRemainingBlocks(transferLength);
                            ...
                            default:
                                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Read CD: unknown format.");
                ...
                case (byte) 0x43: // read toc
                {
                    ...
                    if (getSelectedDrive().getCdRom().isReady())
                    {
                        if (ATAConstants.IS_LOW_LEVEL_CDROM)
                        {
                            ...
                            int startingTrack = getSelectedDriveController().getBuffer()[6];
                            int toc_length;
                        }
                        ...
                        if (ATAConstants.WITH_WIN32)
                        {
                            ...
                            if (ATAConstants.IS_LOW_LEVEL_CDROM)
                            {
                                ...
                                switch (format)
                                {
                                    ...
                                    case 4:
                                        if (msf != 1)
                                        {
                                            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  READ_TOC_EX: msf not set for format " + format + ".");
                                        }
                                    case 0:
                                    case 1:
                                    case 5:
                                        ...
                                        // } else
                                        // {
                                        // initSendAtapiCommand(channel, atapi_command, toc_length, alloc_length);
                                        // readyToSendAtapi(channel);
                                        // }
                                        break;
                                    default:
                                        logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  READ TOC format " + format + " not supported.");
                                        atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.INV_FIELD_IN_CMD_PACKET, true);
                                        raiseInterrupt(channel);
                                }
                            }
                            ...
                            {
                                logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  LOWLEVEL_CDROM not defined.");
                            }
                            ...
                            switch (format)
                            {
                                ...
                                case 2:
                                    if (ATAConstants.IS_LOW_LEVEL_CDROM)
                                    {
                                        // TODO:
                                        /*
                                         * if (!(BX_SELECTED_DRIVE(channel).cdrom.cd->read_toc(BX_SELECTED_CONTROLLER(channel).buffer, &tocLength, msf,
                                         * startingTrack, format))) { atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.INV_FIELD_IN_CMD_PACKET, true);
                                         * raiseInterrupt(channel); } else { initSendAtapiCommand(channel, atapi_command, toc_length, alloc_length);
                                         * readyToSendAtapi(channel); }
                                         */
                                    }
                                    else
                ...
                case (byte) 0xa8: // read (12)
                {
                    ...
                    {
                        // transfer_length = read_16bit(getSelectedDriveController().getBuffer() + 7);
                        // TODO: 7 - offset?
                        transferLength = read16bit(getSelectedDriveController().getBuffer(), 7);

                    }
                    else
                    {
                        // transfer_length = read_32bit(getSelectedDriveController().getBuffer() + 6);
                        // TODO: 6 - offset?
                        transferLength = read32bit(getSelectedDriveController().getBuffer(), 6);
                    }

                    int lba = 0;

                    // int lba = read_32bit(getSelectedDriveController().getBuffer() + 2);
                    // TODO: 2 - offset?
                    lba = read32bit(getSelectedDriveController().getBuffer(), 2);
                    ...
                    if (lba + transferLength > getSelectedDrive().getCdRom().getCapacity())
                    {
                        transferLength = (getSelectedDrive().getCdRom().getCapacity() - lba);
                    }
                    ...
                    if (transferLength <= 0)
                    {
                        atapiCmdNop(channel);
                        raiseInterrupt(channel);
                        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Read with transfer length <= 0, ok (" + transferLength + ")");
                        break;
                    }

                    /*
                     * Ben: I commented this out and added the three lines above. I am not sure this is the correct thing to do, but it seems to work. FIXME: I
                     * think that if the transfer_length is more than we can transfer, we should return some sort of flag/error/bitrep stating so. I haven't
                     * read the atapi specs enough to know what needs to be done though. if (lba + transfer_length > BX_SELECTED_DRIVE(channel).cdrom.capacity) {
                     * atapi_cmd_error(channel, SENSE_ILLEGAL_REQUEST, ASC_LOGICAL_BLOCK_OOR, 1); raise_interrupt(channel); break; }
                     */
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  cdrom: READ LBA " + lba + ", length " + transferLength + ".");

                    // handle command
                    initSendAtapiCommand(channel, atapiCommand, transferLength * 2048, transferLength * 2048, true);
                ...
                case (byte) 0x2b: // seek
                {
                    ...
                    {
                        atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.LOGICAL_BLOCK_OOR, true);
                        raiseInterrupt(channel);
                        break;
                    ...
                    else
                    {
                        ...
                        logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Seek with no LOWLEVEL_CDROM.");
                ...
                case (byte) 0x1e: // prevent/allow medium removal
                {
                    ...
                    if (getSelectedDrive().getCdRom().isReady())
                    {
                        ...
                        boolean newIsLocked = (getSelectedDriveController().getBuffer()[4] & 1) > 0;
                ...
                case (byte) 0x42: // read sub-channel
                {
                    ...
                    else
                    {
                        ...
                        getSelectedDriveController().setBuffer(1, (byte) 0); // audio not supported
                        ...
                        { // !sub_q == header only
                            ...
                            { // UPC or ISRC
                                retLength = 24;
                                getSelectedDriveController().setBuffer(4, (byte) dataFormat);

                                if (dataFormat == 3)
                                {
                                    getSelectedDriveController().setBuffer(5, (byte) 0x14);
                                    getSelectedDriveController().setBuffer(6, (byte) 1);
                                }

                                getSelectedDriveController().setBuffer(8, (byte) 0); // no UPC, no ISRC
                        ...
                        initSendAtapiCommand(channel, atapiCommand, retLength, allocLength, false);
                        readyToSendAtapi(channel);
                    }
                }
                    break;
                ...
                case (byte) 0x46: // ???
                case (byte) 0x4a: // ???
                    logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  ATAPI command " + atapiCommand + " not implemented yet.");

                    atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.ILLEGAL_OPCODE, true);
                    raiseInterrupt(channel);
                    break;

                default:
                    logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Unknown ATAPI command " + atapiCommand + ".");
                    atapiCmdError(channel, SenseType.ILLEGAL_REQUEST, AscType.ILLEGAL_OPCODE, true);
    ...
    /**
     * Init Mode Sense Single.
     * TODO: deal with src what is the type of src? private void initModeSenseSingle(int channel, const void* src, int size)
     * 
     * @param channel
     * @param src
     * @param size
    ...
    private void initModeSenseSingle(int channel, Object[] src, int size)
    {
        ...
        // TODO: if (bx_options.atadevice[channel][BX_HD_THIS channels[channel].drive_select].Ostatus->get () == BX_INSERTED)
        // {
        // getSelectedDriveController().setBuffer(2, (byte)0x12); // media present 120mm CD-ROM (CD-R) data/audio door closed
        //     
        // } else
        // {
        getSelectedDriveController().setBuffer(2, (byte) 0x70); // no media present
        // }
     ...
     * @return
     */
    private int getPacketField(int channelIndex, int bufferIndex, int start, int numBits)
    {
        ...
        byte[] buffer = channels[channelIndex].getSelectedController().getBuffer();

        result = (((buffer)[bufferIndex] >> (start)) & ((1 << (numBits)) - 1)); // TODO: check this operation works with int's
    ...
    private int getPacketWord(int channelIndex, int bufferIndex)
    {
        ...
        // #define get_packet_word(c,b) (((Bit16u)BX_SELECTED_CONTROLLER((c)).buffer[(b)] << 8)
        // | BX_SELECTED_CONTROLLER((c)).buffer[(b)+1])
        int result = ((channels[channelIndex].getSelectedController().getBuffer()[channelIndex] << 8) | channels[channelIndex].getSelectedController()
                .getBuffer()[(channelIndex) + 1]);
     ...
     * @param show
     */
    private void atapiCmdError(int channel, SenseType senseType, AscType ascType, boolean show)
    {
        if (show)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Atapi_cmd_error, for channel " + channel + ", key " + senseType.toString() + ", asc "
                    + ascType.toString() + ".");
        }
        else
        {
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Atapi_cmd_error, for channel " + channel + ", key " + senseType.toString() + ", asc "
                    + ascType.toString() + ".");
        }

        getSelectedDriveController().setErrorRegister(senseType.getValue() << 4);
        getSelectedDriveController().getInterruptReason().setIo(1);
        getSelectedDriveController().getInterruptReason().setCd(1);
        getSelectedDriveController().getInterruptReason().setRel(0);
        getSelectedDriveController().getStatus().setBusy(0);
     ...
     * @param lazy
     */
    private void initSendAtapiCommand(int channel, int command, int reqLength, int allocLength, boolean lazy)
    {
        ...
        if (((getSelectedDriveController().getByteCount() & 1) > 0) && !(allocLength <= getSelectedDriveController().getByteCount()))
        {
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Odd byte count, for channel " + channel + ", to ATAPI command " + command + ",  using "
        ...
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  ATAPI command with zero byte count.");
        }

        if (allocLength < 0)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + "  Allocation length < 0.");
        }
        if (allocLength == 0)
        {
        ...
        getSelectedDriveController().getInterruptReason().setIo(1); // TOOD: check if order is important in IR
        ...
        if (lazy)
        {
            ...
            getSelectedDriveController().setBufferIndex(getSelectedDriveController().getBufferSize());
        }
        else
        {
            getSelectedDriveController().setBufferIndex(0);
        ...
        getSelectedDrive().getAtpi().setDrqBytes(getSelectedDriveController().getByteCount());

        int newTotalBytesRemaining = (reqLength < allocLength) ? reqLength : allocLength;
        getSelectedDrive().getAtpi().setTotalBytesRemaining(newTotalBytesRemaining);
    ...
    /**
     * Set hard disk command 0x1f7 (mch) Writes to the command register with drive_select != 0 are ignored if no secondary device is present
     * 
     * @param originalAddress
     * @param data
     * @param logicalSector
     * @param ret
     */
    private void setHardDiskCommand(int originalAddress, int[] data, Integer logicalSector, int ret)
    {

        if (getSelectedChannel().isSlaveSelected() && !getSelectedChannel().isSlaveDrivePresent())
        {
            return;
        }
        ...
        {
            logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  hard disk: command sent, controller BUSY.");
        }
        ...
        switch (data[0])
        {
            ...
            case 0x10: // CALIBRATE DRIVE
                ...
                {
                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Calibrate drive for channel " + curChannelIndex + ", issued to non-disk, with index "
                ...
                if (getSelectedDrive().getDriveType() == ATADriveType.NONE)
                {
                    ...
                    getSelectedDriveController().setErrorRegister(0x02); // Track 0 not found
                    getSelectedDriveController().getStatus().setBusy(0);
                    getSelectedDriveController().getStatus().setDriveReady(1);
                    getSelectedDriveController().getStatus().setSeekComplete(0);
                    getSelectedDriveController().getStatus().setDrq(0);
                    ...
                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Calibrate drive for channel " + curChannelIndex + ", not present, with index "
                            + getSelectedChannel().getSelectedDrive() + ".");
                    break;
                }
            ...
            case 0xC4: // READ MULTIPLE SECTORS
                ...
                /*
                 * update sector_no, always points to current sector after each sector is read to buffer, DRQ bit set and issue IRQ if interrupt handler
                 * transfers all data words into main memory, and more sectors to read, then set BSY bit again, clear DRQ and read next sector into buffer
                 * sector count of 0 means 256 sectors
                 */

                if (getSelectedDrive().getDriveType() != ATADriveType.HARD_DISK)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Read multiple for channel " + curChannelIndex + ", issued to non-disk, with index "
                            + getSelectedChannel().getSelectedDrive() + ".");
                ...
                if (!(getSelectedDriveController().getLbaMode() > 0) && !(getSelectedDrive().getCurrentHead() > 0)
                        && !(getSelectedDrive().getCurrentCylinder() > 0) && !(getSelectedDrive().getCurrentSector() > 0))
                {
                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  read from 0/0/0, for channel " + curChannelIndex + ", with device index "
                            + getSelectedChannel().getSelectedDrive() + ", aborting command.");
                ...
                if ((byte) channels[this.curChannelIndex].getSelectedController().getCurrentCommand() == (byte) 0xC4)
                {
                    if (channels[this.curChannelIndex].getSelectedController().getMultipleSectors() == 0)
                    {
                        abortCommand(curChannelIndex, data[0]);
                        break;
                    }
                ...
                {
                    channels[this.curChannelIndex].getSelectedController().setBufferSize(512);
                }
            ...
            case 0xC5: // WRITE MULTIPLE SECTORS
                ...
                if (getSelectedDrive().getDriveType() != ATADriveType.HARD_DISK)
                {
                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Write sectors, for channel " + curChannelIndex + ", issued to non-disk with device index "
                            + getSelectedChannel().getSelectedDrive() + ".");
                ...
                getSelectedDriveController().setErrorRegister(0);
                getSelectedDriveController().getStatus().setBusy(0);
                // BX_SELECTED_CONTROLLER(channel).status.drive_ready = 1; //note: commented out in BOCHS code
                getSelectedDriveController().getStatus().setSeekComplete(1);
                getSelectedDriveController().getStatus().setDrq(1);
                getSelectedDriveController().getStatus().setErr(0);
                getSelectedDriveController().setBufferIndex(0);
            ...
            case 0x90: // EXECUTE DEVICE DIAGNOSTIC
                ...
                if (getSelectedDriveController().getStatus().getBusy() > 0)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Diagnostic command, for channel " + curChannelIndex
                            + ", BSY bit set, for disk with device index " + getSelectedChannel().getSelectedDrive() + ".");
            ...
            case 0x91: // INITIALIZE DRIVE PARAMETERS
                ...
                if (getSelectedDriveController().getStatus().getBusy() > 0)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Init drive parameters command, for channel " + curChannelIndex
                            + ", BSY bit set, for disk with device index " + getSelectedChannel().getSelectedDrive() + ".");
                ...
                if (getSelectedDrive().getDriveType() != ATADriveType.HARD_DISK)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Initialize drive parameters, for channel " + curChannelIndex
                            + ", issued to non-disk, with device index " + getSelectedChannel().getSelectedDrive() + ".");
                ...
                logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  Initialize drive parameters, for channel " + curChannelIndex + ", issued to disk, with device index "
                        + getSelectedChannel().getSelectedDrive() + ", sector count= " + getSelectedDrive().getSectorCount() + ", current head= "
                        + getSelectedDrive().getCurrentHead() + ".");

                if (getSelectedDrive().getDriveType() == ATADriveType.NONE)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Initialize drive parameters, for channel " + curChannelIndex
                            + ", disk not present, with device index " + getSelectedChannel().getSelectedDrive());

                    // BX_SELECTED_CONTROLLER(channel).error_register = 0x12; //commented out in BOCHS
                ...
                if (getSelectedDrive().getSectorCount() != getSelectedDrive().getTotalNumSectors())
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Initialize drive parameters, for channel " + curChannelIndex + ", device index "
                            + getSelectedChannel().getSelectedDrive() + " sector count doesn't match.");

                    getSelectedDrive().setSectorCount(getSelectedDrive().getTotalNumSectors());

                    abortCommand(curChannelIndex, data[0]);

                    break;
                }
                if (getSelectedDrive().getCurrentHead() != (getSelectedDrive().getTotalNumHeads() - 1))
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Initialize drive parameters, for channel " + curChannelIndex + ", device index "
                            + getSelectedChannel().getSelectedDrive() + " head number doesn't match.");

                    getSelectedDrive().setCurrentHead((getSelectedDrive().getTotalNumHeads() - 1));
            ...
            case 0xec: // IDENTIFY DEVICE
                ...
                if (getSelectedDrive().getDriveType() == ATADriveType.NONE)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Identify device, for channel " + curChannelIndex + ", disk not present, with device index "
                            + getSelectedChannel().getSelectedDrive() + ", aborting.");
            ...
            case 0xef: // SET FEATURES
                ...
                switch (getSelectedDrive().getFeatures())
                {
                    ...
                    case 0x03: // Set Transfer Mode
                        ...
                        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Set transfer mode, for channel " + curChannelIndex + ", device index "
                                + getSelectedChannel().getSelectedDrive() + ", not supported for subcommand " + getSelectedDrive().getFeatures()
                    ...
                    case 0x66: // Disable reverting to power-on default
                        ...
                        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  SET FEATURES, for channel " + curChannelIndex + ", device index "
                                + getSelectedChannel().getSelectedDrive() + ", not supported for subcommand " + getSelectedDrive().getFeatures()
                                + ", but returning success.");
                    ...
                    default:
                        ...
                        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  SET FEATURES, for channel " + curChannelIndex + ", device index "
                                + getSelectedChannel().getSelectedDrive() + ", with unknown subcommand " + getSelectedDrive().getFeatures() + ".");
            ...
            case 0x40: // READ VERIFY SECTORS
                ...
                if (getSelectedDrive().getDriveType() != ATADriveType.HARD_DISK)
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  read verify, for channel " + curChannelIndex + ", issued to non-disk with device index "
                            + getSelectedChannel().getSelectedDrive() + ".");
                ...
                logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  verify command : 0x40, for channel " + curChannelIndex + ", issued to device with device index "
                        + getSelectedChannel().getSelectedDrive() + ".");
            ...
            case 0xc6: // SET MULTIPLE MODE
                ...
                if (getSelectedDrive().getDriveType() != ATADriveType.HARD_DISK)
                {
                    ...
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  Set multiple mode, for channel " + curChannelIndex
                            + ", issued to non-hard disk, with device index " + getSelectedChannel().getSelectedDrive() + ".");

                    abortCommand(curChannelIndex, data[0]);

                }
                else if ((getSelectedDrive().getSectorCount() > ATAConstants.MAX_MULTIPLE_SECTORS)
                        || ((getSelectedDrive().getSectorCount() & (getSelectedDrive().getSectorCount() - 1)) != 0)
                ...
                else
                {
                    ...
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  set multiple mode: sectors= " + getSelectedDrive().getSectorCount());

                    getSelectedDriveController().setMultipleSectors(getSelectedDrive().getSectorCount());
                    getSelectedDriveController().getStatus().setBusy(0);
                    getSelectedDriveController().getStatus().setDriveReady(1);
                    getSelectedDriveController().getStatus().setWriteFault(0);
                    getSelectedDriveController().getStatus().setDrq(0);
            ...
            case 0x08: // DEVICE RESET (atapi)
                ...
                if (getSelectedDrive().getDriveType() == ATADriveType.CDROM)
                {
                    ...
                    int newErrorReg = getSelectedDriveController().getErrorRegister();
                    newErrorReg &= ~(1 << 7);
                ...
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  ATAPI Device Reset, for channel " + curChannelIndex
                            + ", issued to on non-cd device, with device index " + getSelectedChannel().getSelectedDrive() + ".");
            ...
            case 0xa0: // SEND PACKET (atapi)
                ...
                {
                    // PACKET
                    getSelectedDriveController().setPacketDma(getSelectedDrive().getFeatures() & 1);

                    if ((getSelectedDrive().getFeatures() & (1 << 1)) > 0)
                    {
                        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  PACKET-overlapped not supported, for channel " + curChannelIndex
                                + ", issued to device index " + getSelectedChannel().getSelectedDrive() + ".");
            ...
            case 0xa2: // SERVICE (atapi), optional
                ...
                if (getSelectedDrive().getDriveType() == ATADriveType.CDROM)
                {
                    ...
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  ATAPI SERVICE not implemented, for channel " + curChannelIndex + ", issued to device index "
                            + getSelectedChannel().getSelectedDrive() + ".");
                ...
                if (getSelectedDrive().getDriveType() == ATADriveType.HARD_DISK)
                {
                    ...
                    if (!getSelectedDrive().calculateLogicalAddress(logicalSector))
                    {
                        ...
                        logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  seek, for channel " + curChannelIndex + ", issued to device index "
                                + getSelectedChannel().getSelectedDrive() + "initial seek to sector " + logicalSector + "out of bounds, aborting.");
                    ...
                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  SEEK disable ird set to " + getSelectedChannel().getDrives()[0].getControl().isDisableIrq()
                            + ", for channel " + curChannelIndex + ", with device index " + 0 + ".");

                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  SEEK disable ird set to " + getSelectedChannel().getDrives()[1].getControl().isDisableIrq()
                            + ", for channel " + curChannelIndex + ", with device index " + 1 + ".");

                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  SEEK completed. error_register " + getSelectedDriveController().getErrorRegister()
                            + ", for channel " + curChannelIndex + ", with device index " + getSelectedChannel().getSelectedDrive() + ".");

                    raiseInterrupt(curChannelIndex);

                    logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + "  SEEK interrupt completed, for channel " + curChannelIndex + ", issued to device index "
                            + getSelectedChannel().getSelectedDrive());
                }
                else
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  write cmd 0x70 (SEEK), for channel " + curChannelIndex
                            + ", not supported for non-disk with device index " + getSelectedChannel().getSelectedDrive() + ".");
...
/*                if (dma != null)
                ...
                {
*/                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  write cmd 0xC8 (READ DMA) not supported, for channel " + curChannelIndex + ", device index "
                            + getSelectedChannel().getSelectedDrive() + ".");
...
/*                if (dma != null)
                ...
                {
*/
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  write cmd 0xCA (WRITE DMA) not supported, for channel " + curChannelIndex + ", device index "
                            + getSelectedChannel().getSelectedDrive() + ".");
...
//                }
            ...
            default:
                logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + "  IO write to " + originalAddress + ", unknown command " + data + " for channel " + curChannelIndex
                        + ", device index " + getSelectedChannel().getSelectedDrive() + ".");
                abortCommand(curChannelIndex, data[0]);
        }
    ...
    private void identifyAtapiDrive(int channel)
    {
        ...
        getSelectedDrive().setIdDrive(0, ((2 << 14) | (5 << 8) | (1 << 7) | (2 << 5) | (0 << 0))); // Removable CDROM, 50us response, 12 byte packets
...
/*        if (dma != null)
        {
            getSelectedDrive().setIdDrive(49, ((1 << 9) | (1 << 8))); // LBA and DMA
        }
        else
        {
*/
...
//        }
        ...
        for (i = 0; i <= 255; i++)
        {
            ...
            getSelectedDriveController().setBuffer(i * 2, (byte) (temp16 & 0x00ff));
        }
    }

    /**
     * Identify the drive by setting the ID Drive params and writing the data to the buffer.
     * 
     * @param channel the channel
     */
    private void identifyDrive(int channel)
    {
        ...
        if (ATAConstants.IS_CONNER_CFA540A_DEFINED)
        {
            ...
            getSelectedDrive().setIdDrive(3, getSelectedDrive().getTotalNumHeads());
            getSelectedDrive().setIdDrive(4, 0x9fb7);
            getSelectedDrive().setIdDrive(5, 0x0289);
            getSelectedDrive().setIdDrive(6, getSelectedDrive().getTotalNumSectors());
            ...
            getSelectedDrive().setIdDrive(47, 0x8080); // multiple mode identification
            ...
            getSelectedDrive().setIdDrive(55, getSelectedDrive().getTotalNumHeads());
            getSelectedDrive().setIdDrive(56, getSelectedDrive().getTotalNumSectors());
            getSelectedDrive().setIdDrive(57, 0x1e80);
            getSelectedDrive().setIdDrive(58, 0x0010);
            getSelectedDrive().setIdDrive(59, (0x0100) | getSelectedDriveController().getMultipleSectors());
        ...
        else
        {
            ...
            getSelectedDrive().setIdDrive(1, getSelectedDrive().getTotalNumCylinders());
            ...
            getSelectedDrive().setIdDrive(4, (512 * getSelectedDrive().getTotalNumSectors()));
            ...
            getSelectedDrive().setIdDrive(47, ATAConstants.MAX_MULTIPLE_SECTORS);
            ...
            getSelectedDrive().setIdDrive(54, getSelectedDrive().getTotalNumCylinders());
            getSelectedDrive().setIdDrive(55, getSelectedDrive().getTotalNumHeads());
            getSelectedDrive().setIdDrive(56, getSelectedDrive().getTotalNumSectors());
            ...
            temp32 = getSelectedDrive().getTotalNumCylinders() * getSelectedDrive().getTotalNumHeads() * getSelectedDrive().getTotalNumSectors();
            ...
            int num_sects = getSelectedDrive().getTotalNumCylinders() * getSelectedDrive().getTotalNumHeads() * getSelectedDrive().getTotalNumSectors();
        ...
        {
            temp16 = getSelectedDrive().getIdDrive()[i];
            getSelectedDriveController().setBuffer(i * 2 + 1, (byte) (temp16 & 0x00ff));
    ...
    private int read32bit(byte[] buf, int offset)
    {
        ...
        int returnValue = (buf[0 + offset] << 24) | (buf[1 + offset] << 16) | (buf[2 + offset] << 8) | buf[3 + offset];
