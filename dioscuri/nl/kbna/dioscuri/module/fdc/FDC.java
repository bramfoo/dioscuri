...
public class FDC extends ModuleFDC
{
    ...
    {
        // Register I/O ports 03F0-03F7 - Floppy Disk Controller in I/O address space
    ...
    public void stop()
    {
        // Make sure all data on virtual floppies in drives are stored to image files
        for (int i = 0; i < drives.length; i++)
        {
...
//                      logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + " WRITE DONE: drive=" + drive + ", hds=" + drives[drive].hds + ", cylinder=" + drives[drive].cylinder + ", sector=" + drives[drive].sector);
        ...
        switch (portAddress)
        {
            ...
            case 0x03F5: // Diskette controller data
                ...
                else
                {
                    ...
                    resultIndex++;
                    // Clear MSR bits to indicate that drives are not busy anymore (LSB)
                    msr &= 0xF0;
                    // Clear interrupt
                    this.clearInterrupt();
    ...
    {
        // Try to eject the floppy by removing reference (will be carbage collected)
        try
        {
            if (driveIndex != -1)
            {
        ...
        switch (commandPending)
        {
            ...
            case 0xE6: // read normal data, MT=1, SK=1
                ...
                if (hds != ((command[1] >> 2) & 0x01))
                {
                    ...
                    ableToTransfer = false;
                    statusRegister0 = 0x40 | (drives[drive].hds << 2) | drive; // abnormal termination
                ...
                {
                    // Check if end of track is 0, if so assign total sectors per track as upper boundary
                    if (eot == 0)
                    {
            ...
            case 0x07: // Recalibrate (2 bytes)
                ...
                // FDC is busy, 1=active, 0=not active (this is different from what Bochs does!!!)
                msr = (byte) (1 << drive);
                break;
    ...
    protected void setDMAByte(byte data)
    {
        ...
        else
        {
            ...
            if ((floppyBufferIndex >= 512) || (tc == true))
            {
                ...
                {
                    drives[drive].writeData(logicalSector * 512, 512, floppyBuffer);
                    drives[drive].incrementSector(); // increment to next sector after writing current one
