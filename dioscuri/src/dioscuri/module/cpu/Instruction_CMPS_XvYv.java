...
public class Instruction_CMPS_XvYv implements Instruction {
    ...
    /**
     * Compare word at address DS:(E)SI with address ES:(E)DI; set flags accordingly.<BR>
    ...
    public void execute()
    {
        ...
        // Test AF
        cpu.flags[CPU.REGISTER_FLAGS_AF] = Util.test_AF_SUB(source1[CPU.REGISTER_GENERAL_LOW], result[CPU.REGISTER_GENERAL_LOW]);
        // Test CF
        ...
        cpu.flags[CPU.REGISTER_FLAGS_ZF] = result[CPU.REGISTER_GENERAL_HIGH] == 0x00 && result[CPU.REGISTER_GENERAL_LOW] == 0x00? true : false;
        // Test SF on particular byte of destinationRegister (set when MSB is 1, occurs when destReg >= 0x80)
        cpu.flags[CPU.REGISTER_FLAGS_SF] = result[CPU.REGISTER_GENERAL_HIGH] < 0 ? true : false;
        // Test PF on particular byte of destinationRegister
        cpu.flags[CPU.REGISTER_FLAGS_PF] = Util.checkParityOfByte(result[CPU.REGISTER_GENERAL_LOW]);
