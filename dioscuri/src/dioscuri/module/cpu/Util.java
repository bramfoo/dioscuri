...
public class Util {
	...
	// Attributes
	private static boolean[] parityLookupByte = {
...
//    protected static byte[] result = new byte[2];
     ...
     * Takes care of possible carry from LSB to MSB. Not beyond MSB.
     * Does not care about signed/unsigned as this doesn't effect the actual result in hex.
     * 
    ...
    protected static byte[] addWords(byte[] word1, byte[] word2, int carryBit)
    {
        ...
        lowerByteCarry = ((int)word1[CPU.REGISTER_LOW] & 0xFF) + ((int)word2[CPU.REGISTER_LOW] & 0xFF) + carryBit;
     ...
     * Takes care of possible borrow from LSB to MSB. Not beyond MSB.
     * Does not care about signed/unsigned as this doesn't effect the actual result in hex.
     * Operation: word1 - (word2 + borrow)
     * 
     * @param   word1    first word
     * @param   word2    second word
     * @param   int      borrow bit
     * 
     * @return  subtraction of both words plus borrow including possible borrow between LSB and MSB
     */
     ...
     * @param   disp    displacement following ModR/M byte
	 * 
	...
	{
		byte[] result = new byte[2];
        result[CPU.REGISTER_GENERAL_LOW]  = (byte) (reg1[CPU.REGISTER_GENERAL_LOW]  + reg2[CPU.REGISTER_GENERAL_LOW]  + displacement[CPU.REGISTER_GENERAL_LOW]);
		result[CPU.REGISTER_GENERAL_HIGH] = (byte) (reg1[CPU.REGISTER_GENERAL_HIGH] + reg2[CPU.REGISTER_GENERAL_HIGH] + displacement[CPU.REGISTER_GENERAL_HIGH]);
		// Check for overflow; because adding 3 registers, overflow of 2 possible!
		int intermediateResult = (((int)reg1[CPU.REGISTER_GENERAL_LOW])&0xFF)  + (((int)reg2[CPU.REGISTER_GENERAL_LOW])&0xFF) + (((int)displacement[CPU.REGISTER_GENERAL_LOW])&0xFF);
     ...
     *  
     * @param   unsigned byte input
     * @param   unsigned byte result
     * 
     ...
     * This happens when last shift causes a carry out of lowest nibble
     *  
     * @param   unsigned byte input
     * @param   shifts
     * 
    ...
    {
        // Check if number of shifts is in range of bits in lowest nibble, else no carry happens
     ...
     * This happens when we cross over from 255 to 0 while adding or cross over from 0 to 255 while subtracting.
     * The operation is (input1 + carry) + input2
     *  
     * @param   unsigned byte input1
     * @param   unsigned byte input2
     * @param   carry
     * 
     * @return  true if carry occurs, false otherwise
     */
     ...
     * CF is set when unsigned overflow occurs.
     * This happens when we cross over from 65535 (0xFFFF) to 0 while adding or cross over from 0 to 65535 (0xFFFF) while subtracting.
     * The operation is (input1 + carry) + input2
     *  
    ...
    protected static boolean test_CF_ADD(byte[] input1, byte[] input2, int carry)
    {
        ...
        if (((((int)input1[CPU.REGISTER_GENERAL_HIGH]) & 0xFF) << 8) + (((int)input1[CPU.REGISTER_GENERAL_LOW]) & 0xFF) + 
                ((((int)input2[CPU.REGISTER_GENERAL_HIGH]) & 0xFF) << 8) + (((int)input2[CPU.REGISTER_GENERAL_LOW]) & 0xFF) + carry > (((int) 0xFFFF) & 0xFFFF))
        {
            return true;
        }
        return false;
...
//        if ((((int) input1) & 0xFF) + carry < (((int) input2) & 0xFF))
    ...
    protected static boolean test_CF_SUB(byte[] input1, byte[] input2, int carry)
    {
        ...
        if (((((int)input1[CPU.REGISTER_GENERAL_HIGH]) & 0xFF) << 8) + (((int)input1[CPU.REGISTER_GENERAL_LOW]) & 0xFF) < 
                ((((int)input2[CPU.REGISTER_GENERAL_HIGH]) & 0xFF) << 8) + (((int)input2[CPU.REGISTER_GENERAL_LOW]) & 0xFF) + carry)
        {
            return true;
        }
        return false;
     ...
     * This happens when we cross over from 127 to -128 while adding or cross over from -128 to 127 while subtracting.
     *  
    ...
    protected static boolean test_OF_ADD(byte input1, byte input2, byte result, int carry)
    {
        ...
        if (result >= 0)
        {
            // Result (signed) positive, Input1 and Input2 must be (signed) negative to set OF true
        ...
        {
            // Result (signed) negative, Input1 and Input2 must be (signed) positive to set OF true
            if (input1 + carry >= 0 && input2 >= 0)
                return true;
            else
                return false;
     ...
     * OF is set when 2's complement signed overflow occur.
     * This happens when we cross over from 32767 (0x7FFF) to -32768 (0x8000) while adding or cross over from -32768 to 32767 while subtracting.
     *  
     * @param   byte[]  signed input1 of operation
     * @param   byte[]  signed input2 of operation
     * @param   byte[]  signed result of operation
    ...
    protected static boolean test_OF_ADD(byte[] input1, byte[] input2, byte[] result, int carry)
    {
        ...
        // (b) (negative) + (negative) = (positive)
        // Test these two possibilities here:
        if ((((int)result[CPU.REGISTER_GENERAL_HIGH]) << 8) + (((int)result[CPU.REGISTER_GENERAL_LOW] & 0xFF)) >= 0)
        {
            // Result (signed) positive, Input1 and Input2 must be (signed) negative to set OF true
            if ((((int)input1[CPU.REGISTER_GENERAL_HIGH]) << 8) + (((int)input1[CPU.REGISTER_GENERAL_LOW]) & 0xFF) + carry < 0 && 
                    (((int)input2[CPU.REGISTER_GENERAL_HIGH]) << 8) + (((int)input2[CPU.REGISTER_GENERAL_LOW]) & 0xFF) < 0)
                return true;
            else
                return false;
        }
    ...
    protected static boolean test_OF_SUB(byte input1, byte input2, byte result, int carry)
    {
        ...
        if (input1 >= 0)
        {
            // Input1 (signed) positive, Input2 and Result must be (signed) negative to set OF true
        ...
        {
            // Input1 (signed) negative, Input2 and Result must be (signed) positive to set OF true
            if (input2 + carry >= 0 && result >= 0)
                return true;
            else
                return false;
    ...
    protected static boolean test_OF_SUB(byte[] input1, byte[] input2, byte[] result, int carry)
    {
        ...
        // (b) (negative) - (positive) = (positive)
        // Test these two possibilities here:
        if ((((int)input1[CPU.REGISTER_GENERAL_HIGH]) << 8) + (((int)input1[CPU.REGISTER_GENERAL_LOW]) & 0xFF) >= 0)
        {
            // Input1 (signed) positive, Input2 and Result must be (signed) negative to set OF true
            if ((((int)input2[CPU.REGISTER_GENERAL_HIGH]) << 8) + (((int)input2[CPU.REGISTER_GENERAL_LOW]) & 0xFF) + carry < 0 && 
                    (((int)result[CPU.REGISTER_GENERAL_HIGH]) << 8) + (((int)result[CPU.REGISTER_GENERAL_LOW]) & 0xFF) < 0)
                return true;
            else
                return false;
        }
    ...
    {
        // Depending on sign from input byte, return either 0x00 (positive) or 0xFF (negative)
     ...
     * Each boolean value is converted into a hexadecimal (0 or 1) value and placed in a byte[].
     * Note: boolean[] must be of multiple of 8 booleans.
     * 
    ...
    protected static byte[] booleansToBytes(boolean[] booleans)
    {
        ...
        int[] tempHex = new int[booleans.length / 8]; // Assumes boolean array is multiple of 8
        ...
        for (int b = tempHex.length - 1; b >= 0; b--)
        {
            for (int j = 0; j <= 7; j++)
            {
                ...
                tempValue = booleans[j + ((tempHex.length - 1 - b) * 8)] ? 1 : 0;
     ...
     * Each bit is converted into a boolean value and placed in a boolean[].
     * 
    ...
    {
        return (Integer.toHexString(0x100 | (word[0] & 0xFF)).substring(1) + Integer.toHexString(0x100 | (word[1] & 0xFF)).substring(1));
    }
    ...
    protected static byte convertStringToByte(String strValue)
    {
        ...
        try
        {
            ...
            {
                intRegVal = (byte) (intRegVal + ((int)Math.pow(16, strValue.length() - i ))* Integer.parseInt(strValue.substring(i-1,i),16));
            }
    ...
    protected static byte[] convertStringToWord(String strValue)
    {
        ...
        try
        {
            ...
            for (int i = strValue.length(); i > 0; i--)
            {
                intRegVal = (byte) (intRegVal + Math.pow(16, strValue.length() - i ) * Byte.parseByte(strValue.substring(i-1,i),16));
