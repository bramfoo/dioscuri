...
package dioscuri.module.cpu;
	 ...
	 * Intel opcode 69<BR>
	 * Signed multiply.<BR>
	 * Multiplication uses three operands: 1=destination, 2=first source, 3=second source<BR>
	 * Flags modified: CF, OF. Flags SF, ZF, AF, and PF are undefined
	 */
public class Instruction_IMUL_GvEvIv implements Instruction {
	...
	public void execute()
	{
        ...
        if (cpu.doubleWord)
        {
        	...
        	System.out.println("[cpu] Instruction IMUL 32-bit not implemented");
        }
        else
        {
        	// 16-bit operand size
            ...
            if (((addressByte >> 6) & 0x03) == 3)
            {
                ...
                sourceWord1 = cpu.decodeRegister(operandWordSize, addressByte & 0x07);
            }
            else
            {
                // IMUL mem, imm
                ...
                sourceWord1 = cpu.getWordFromMemorySegment(addressByte, memoryReferenceLocation);
            }
        	
            // Retrieve source1 imm
            sourceWord2 = cpu.getWordFromCode();
        }
        ...
        // Flags: OF, CF set if result has to be truncated to fit in destination. SF, ZF, AF, PF are undefined.
