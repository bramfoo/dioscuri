...
/**
 * Intel opcode 10<BR>
 * Add byte (+ CF) in register (source) to memory/register (destination).<BR>
 * The addressbyte determines the source (rrr bits) and destination (sss bits).<BR>
 * Flags modified: OF, SF, ZF, AF, PF, CF
 */
public class Instruction_ADC_EbGb implements Instruction {
     ...
     */
    public Instruction_ADC_EbGb(CPU processor) {
        this();
    ...
    public void execute() {
        ...
        if (((addressByte >> 6) & 0x03) == 3) {
            ...
            // Get byte from memory and ADC source register + CF
            destinationValue = (byte) (sourceValue2 + (sourceValue + iCarryFlag));
