...
public class Instruction_INCDEC_GRP5 implements Instruction
{
    ...
    /**
     * Execute any of the following Immediate Group 5 instructions: INC, DEC, CALLN, CALLF, JMPN, JMPF, PUSH.<BR>
     */
    public void execute()
    {
        // Get addresByte
        ...
        switch ((addressByte & 0x38) >> 3)
        {
            ...
            case 2: // CALLN Ev
                // Call near is absolute indirect, address given in register/memory (r/m16, r/m32) 
                ...
                else
                {
                    ...
                    newIP = cpu.getWordFromMemorySegment(addressByte, memoryReferenceLocation);
                }
            ...
            case 3: // CALLF Ep
                // Call far is absolute indirect, address given in memory (IP:CS=m16:16 or m16:32) 
                
                if (((addressByte >> 6) & 0x03) == 3)
                {
                    // Address given in register
                    // TODO: is this case possible?
                    logger.log(Level.WARNING, "[" + cpu.getType() + "]" + " Instruction INCDEC_GRP5 (0xFF): CALLF in unsupported case.");
                }
                else
                {
                    ...
                    // Determine CS displacement of memory location (increment disp with 2, because of 16-bit IP) 
                    temp = Util.addWords(new byte[] {0x00, 0x02}, memoryReferenceDisplacement, 0);
                    System.arraycopy(temp, 0, memoryReferenceDisplacement, 0, temp.length);

                    // Determine memory location
                    memoryReferenceLocation = cpu.decodeSSSMemDest(addressByte, memoryReferenceDisplacement);

                    // Get new CS from memory
                    newCS = cpu.getWordFromMemorySegment(addressByte, memoryReferenceLocation);
            ...
            case 5: // JMPF Ep
                ...
                if (((addressByte >> 6) & 0x03) == 3)
                {
                    // Address given in register
                    // TODO: is this case possible?
                    logger.log(Level.WARNING, "[" + cpu.getType() + "]" + " Instruction INCDEC_GRP5 (0xFF): JMPF in unsupported case.");
                }
                else
                {
                    // Address given in memory (m16:16 or m16:32)
            ...
            case 6: // PUSH Ev: push word or doubleword onto the stack
                ...
                // FIXME: take notice of the stack address size (16 / 32) and doubleWord
                ...
                // Retrieve source value from memory and push it onto stack (also takes care of decrementing SP)
                cpu.setWordToStack(cpu.getWordFromMemorySegment(addressByte, memoryReferenceLocation));
                break; // PUSH Ev
    
            default:
                logger.log(Level.SEVERE, "[" + cpu.getType() + "]" + " Instruction INCDEC_GRP5 (0xFF): no group instruction match.");
                break;
        }
    }
}
