        ...
        switch ((addressByte & 0x38) >> 3)
        {
            case 0: // ADD
                ...
                if (((addressByte >> 6) & 0x03) == 3)
                {
                    ...
                    registerHighLow = ((addressByte & 0x4) >> 2) == 0 ? (byte) CPU.REGISTER_GENERAL_LOW : (byte) CPU.REGISTER_GENERAL_HIGH;
            ...
            case 1: // OR
                ...
                else
                {
                    ...
                    destByte = cpu.getByteFromMemorySegment(addressByte, memoryReferenceLocation);
                    destByte |= sourceByte;
                    // Store result in memory
                    ...
                    // Set PF on result
                    cpu.flags[CPU.REGISTER_FLAGS_PF] = Util.checkParityOfByte(destByte);
            ...
            case 2: // ADC
                ...
                else
                {
                    ...
                    // Store result in memory
                    cpu.setByteInMemorySegment(addressByte, memoryReferenceLocation, tempResultByte);
                    
                    // Test AF
                    cpu.flags[CPU.REGISTER_FLAGS_AF] = Util.test_AF_ADD(destByte, tempResultByte);
                    // Test CF
                    cpu.flags[CPU.REGISTER_FLAGS_CF] = Util.test_CF_ADD(destByte, sourceByte, iCarryFlag);
                    // Test OF
                    cpu.flags[CPU.REGISTER_FLAGS_OF] = Util.test_OF_ADD(destByte, sourceByte, tempResultByte, 0);
                    // Test ZF on result
                    cpu.flags[CPU.REGISTER_FLAGS_ZF] = tempResultByte == 0 ? true : false;
                    // Test SF on result (set when MSB is 1, occurs when result >= 0x80)
                    cpu.flags[CPU.REGISTER_FLAGS_SF] = tempResultByte < 0 ? true : false;
                    // Set PF on result
                    cpu.flags[CPU.REGISTER_FLAGS_PF] = Util.checkParityOfByte(tempResultByte);
                }
                break;  // ADC
