...
/**
 * Intel opcode 1B<BR>
 * Subtract word in memory/register (source) + CF from register (destination).<BR>
 * The addressbyte determines the source (rrr bits) and destination (sss bits).<BR>
 * Flags modified: OF, SF, ZF, AF, PF, CF
 */
public class Instruction_SBB_GvEv implements Instruction {
    ...
    public void execute() {
        ...
        // Test CF
        cpu.flags[CPU.REGISTER_FLAGS_CF] = Util.test_CF_SUB(oldDest, oldSource,
                iCarryFlag);
        // Test OF
        cpu.flags[CPU.REGISTER_FLAGS_OF] = Util.test_OF_SUB(oldDest, oldSource,
                destinationRegister, iCarryFlag);
