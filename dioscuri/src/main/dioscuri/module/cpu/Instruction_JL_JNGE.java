...
/**
 * Intel opcode 7C<BR>
 * Conditional short jump if sign != overflow.<BR>
 * Displacement is relative to next instruction.<BR>
 * Flags modified: none
 */
public class Instruction_JL_JNGE implements Instruction
{
     ...
     */
    public Instruction_JL_JNGE(CPU processor)
    {
        // this();
    ...
    public void execute()
    {
        ...
        // Jump if sign flag is not equal to overflow flag, otherwise skip instruction
        // IP has already been properly updated when bytes were retrieved
        if (cpu.flags[CPU.REGISTER_FLAGS_SF] != cpu.flags[CPU.REGISTER_FLAGS_OF])
        {
