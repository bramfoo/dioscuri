...
/**
 * Intel opcode 8F<BR>
 * Pop word or double word from stack SP:SS into current segment at given offset
 * (mem/reg).<BR>
 * NOTE: Stack is incr. automatically NOTE: The POP instruction cannot pop a
 * value into the CS register. To load the CS register from the stack, use the
 * RET instruction. Flags modified: none
 */
public class Instruction_POP_Ev implements Instruction {
    ...
    /**
     * This pops the word or doubleword at stack top SS:SP into current segment
     * at given offset (reg/mem) NOTE: Stack is incr. automatically NOTE: The
    ...
    public void execute() {
        ...
        if (cpu.doubleWord) {
            ...
            // Determine if offset is given in register or memory (by mm bits of
            // addressbyte)
            if (((addressByte >> 6) & 0x03) == 3) {
                // reg offset
                // Determine register from addressbyte, ANDing it with 0000 0111
                offset = cpu
                        .decodeRegister(operandWordSize, addressByte & 0x07);
            } else {
                // mem offset
                // Determine memory location
                offset = cpu.decodeSSSMemDest(addressByte,
                        memoryReferenceDisplacement);
            }

            // Pop word from stack and store it in segment + offset
            cpu.setWordInMemorySegment(addressByte, offset, cpu
                    .getWordFromStack());
        }
    }
}
