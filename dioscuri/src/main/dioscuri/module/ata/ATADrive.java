...
public class ATADrive {
    ...
    public ATADrive(ATADriveType theDriveType, ATA parent, boolean enabled,
        ...
        if (theDriveType == ATADriveType.CDROM) {
            ...
            // logger.log(Level.INFO, this.getType() + " -> "CDROM on target,
            // channel .");
             ...
             * BX_CONTROLLER(channel,device).interrupt_reason.c_d = 1; if
             * (BX_CONTROLLER(channel,device).sector_count != 0x01) {
             * BX_PANIC(("interrupt reason bit field error")); }
             * 
             * BX_CONTROLLER(channel,device).sector_count = 0;
             * BX_CONTROLLER(channel,device).interrupt_reason.i_o = 1; if
             * (BX_CONTROLLER(channel,device).sector_count != 0x02) {
             * BX_PANIC(("interrupt reason bit field error")); }
             * 
             * BX_CONTROLLER(channel,device).sector_count = 0;
             * BX_CONTROLLER(channel,device).interrupt_reason.rel = 1; if
             * (BX_CONTROLLER(channel,device).sector_count != 0x04) {
             * BX_PANIC(("interrupt reason bit field error")); }
             * 
             * BX_CONTROLLER(channel,device).sector_count = 0;
             * BX_CONTROLLER(channel,device).interrupt_reason.tag = 3; if
             * (BX_CONTROLLER(channel,device).sector_count != 0x18) {
             * BX_PANIC(("interrupt reason bit field error")); }
             */
            this.sectorCount = 0;
    ...
    public boolean calculateLogicalAddress(int sector) {
        ...
        if (this.getControl().getLbaMode() > 0) {
            logicalSector = (this.getCurrentHead()) << 24
                    | (this.getCurrentCylinder()) << 8
                    | this.getCurrentSector();
        } else {
            logicalSector = (this.getCurrentCylinder()
                    * this.getTotalNumHeads() * this.getTotalNumSectors())
        ...
        if (this.getControl().getLbaMode() > 0) {
            ...
            if (this.getCurrentSector() > this.getTotalNumSectors()) {
                ...
                if (this.getCurrentHead() >= this.getTotalNumHeads()) {
                    ...
                    if (this.getCurrentCylinder() >= this
    ...
    protected long getDiskCapacity() {
        ...
        int totalDiskCapacity = totalNumCylinders * totalNumHeads
                * totalNumSectors * control.getNumSectorsPerBlock();
    ...
    /**
     * Set the disk capacity, by input of the number of cylinders, the number of
     * heads, the number of sectors and the number of sectors per block.
     * 
     * @param theTotalNumCylinders
     *            the total number of cylinders
     * @param theTotalNumHeads
     *            the total number of heads
     * @param theTotalNumSectors
     *            the total number of sectors
     * @param theNumSectorsPerBlock
     *            the number of sectors per block
     */
    public void setDiskCapacity(int theTotalNumCylinders, int theTotalNumHeads,
            int theTotalNumSectors, int theNumSectorsPerBlock) {
     ...
     * 
     * @param isProtected
     *            true for write protected, false for not writed protected
     */
    ...
    public void writeData(byte[] diskBuffer, int offset, int totalBytes)
        ...
        if (this.containsDisk() && isWriteProtected == false) {
            ...
            throw new IOException(
                    "Error: drive does not contain a disk or is write protected");
        }
    }
    ...
    public String toString() {
        return "Drive status info:" + "  drivetype=" + driveType.toString()
                ...
                + this.isWriteProtected + ", currentHead=" + currentHead
                + ", currentCylinder=" + currentCylinder + ", currentSector="
                + currentSector + ", numberHeads=" + totalNumHeads
                + ", numberCylinders=" + totalNumCylinders + ", numberSectors="
                + totalNumSectors;
    }
