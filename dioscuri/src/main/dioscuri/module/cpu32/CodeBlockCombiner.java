...
public class CodeBlockCombiner {
    ...
    private RealModeCodeBlock combineCodeBlocks(ByteSourceWrappedMemory source) {
            ...
            while (source0.getNext()) {
                ...
                for (int j = 0; j < operationLength; j++) {
                    if (i == microcodes.length) {
                        ...
                        System.arraycopy(microcodes, 0, temp, 0,
                                microcodes.length);
                        microcodes = temp;
                    }
        ...
        if ((jumpType != MicrocodeSet.JZ_O8)
                && (jumpType != MicrocodeSet.JNZ_O8)
                && (jumpType != MicrocodeSet.JUMP_O8)
                && (jumpType != MicrocodeSet.CALL_O16_A16)) {
            // System.out.println("wrong jump");
            return block0;
        }
        // check jump doesn't go before the start of the first block in this
        // tree of combined blocks
        ...
        if ((block1 instanceof SpanningCodeBlock)
                || (block2 instanceof SpanningCodeBlock))
            return block0;
        ...
        CombiningRealCodeBlock combinedBlock = new CombiningRealCodeBlock(
                block0, block1, block2, start, jumpSize);
        // System.out.println("*********************************************************made combined block");

        // System.out.println("b0:" + start + "," + block0.getX86Length()+
        // " b1:" + (start+block0.getX86Length()) + ","+
        // block1.getX86Length()+" b2:" + (start + block0.getX86Length() +
        // jumpSize) +"," + block2.getX86Length());
