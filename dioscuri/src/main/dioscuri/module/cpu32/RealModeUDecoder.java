...
public final class RealModeUDecoder implements MicrocodeSet, Decoder, InstructionSource {
    ...
    private static final boolean[] sibArray = new boolean[]{ // true for modrm
            // values that
            // require a sib
            // byte (32 bit
            // addressing
    ...
    private static final boolean[] twoByte_0f_modrmArray = new boolean[]{ // true
            // for
            // opcodes
            // that
            // require
    ...
    private static final boolean[] twoByte_0f_sibArray = new boolean[]{ // true
            // for
            // modrm
            // values
            // that
    ...
    private int decodeOpcode()
    {
        ...
        switch (operationHasDisplacement(prefices, opcode, modrm, sib)) {
            ...
            case 1:
                ...
                break;
            case 2:
                displacement = (source.getByte() & 0xff)
                        | ((source.getByte() << 8) & 0xff00);
                bytesRead += 2;
                break;
            case 4:
                displacement = (source.getByte() & 0xff)
                        | ((source.getByte() << 8) & 0xff00)
                        | ((source.getByte() << 16) & 0xff0000)
                        | ((source.getByte() << 24) & 0xff000000);
                bytesRead += 4;
                break;
            default:
        ...
        switch (operationHasImmediate(prefices, opcode, modrm)) {
            ...
            case 1:
                ...
                break;
            case 2:
                immediate = (source.getByte() & 0xff)
                        | ((source.getByte() << 8) & 0xff00);
                bytesRead += 2;
                break;
            case 3:
                immediate = ((source.getByte() << 16) & 0xff0000)
                        | ((source.getByte() << 24) & 0xff000000)
                        | (source.getByte() & 0xff);
                bytesRead += 3;
                break;
            case 4:
                ...
                break;
            case 6:
                immediate = 0xffffffffl & ((source.getByte() & 0xff)
                        | ((source.getByte() << 8) & 0xff00)
                        | ((source.getByte() << 16) & 0xff0000) | ((source
        ...
        writeInputOperands(prefices, opcode, modrm, sib, displacement,
                immediate);
    ...
    private void writeOperation(int prefices, int opcode, int modrm)
    {
        switch (opcode) {
            ...
            case 0x60: // PUSHA/D
                switch (prefices & (PREFICES_OPERAND | PREFICES_ADDRESS)) {
                    case 0:
                        working.write(PUSHA_A16);
                        break;
                    case PREFICES_OPERAND:
                        working.write(PUSHAD_A16);
                        break;
                    case PREFICES_ADDRESS:
                        working.write(PUSHA_A32);
                        break;
                    case PREFICES_ADDRESS | PREFICES_OPERAND:
            ...
            default:
                throw new IllegalStateException("Missing Operation: 0x"
    ...
    private void writeFlags(int prefices, int opcode, int modrm)
    {
        switch (opcode) {
            ...
            default:
                throw new IllegalStateException("Missing Flags: 0x"
                        + Integer.toHexString(opcode));
        }
    }
                                     ...
                                     int sib, int displacement)
    {
        ...
        switch (opcode) {
            ...
            case 0xfb0: // CMPXCHG Eb, Gb
                working.write(STORE1_AL); // do store 1 first incase Eb is also
                // AL/AX/EAX
                store0_Eb(prefices, modrm, sib, displacement);
                break;
            ...
            case 0xfb1: // CMPXCHG Ev, Gv
                if ((prefices & PREFICES_OPERAND) != 0) {
                    working.write(STORE1_EAX); // do store1 first incase Eb is same
                    // place
                    store0_Ed(prefices, modrm, sib, displacement);
                } else {
                    working.write(STORE1_AX); // do store1 first incase Eb is same
                    // place
                    store0_Ew(prefices, modrm, sib, displacement);
                }
                break;
            ...
            case 0xf00: // Group 6
                switch (modrm & 0x38) {
                    case 0x00: // SLDT
                        store0_Ew(prefices, modrm, sib, displacement);
                        break;
                    case 0x08: // STR (stores to a doubleword if a register, but a word
                        // if memory)
                        if ((prefices & PREFICES_OPERAND) != 0) {
                            switch (modrm & 0xc7) {
                                default:
            ...
            case 0xfc0: // XADD Eb, Gb
                store1_Gb(modrm); // exchange first then add (so we write the result
                // of the exchange first incase Eb and Gb are same
                // reg)
                store0_Eb(prefices, modrm, sib, displacement);
                break;
    ...
    private void store0_Cd(int modrm)
    {
        switch (modrm & 0x38) {
            case 0x00:
                ...
                break;
            case 0x10:
                working.write(STORE0_CR2);
                break;
            case 0x18:
                working.write(STORE0_CR3);
                break;
            case 0x20:
                working.write(STORE0_CR4);
                break;
            default:
    ...
    private void load0_Cd(int modrm)
    {
        switch (modrm & 0x38) {
            case 0x00:
                ...
                break;
            case 0x10:
                working.write(LOAD0_CR2);
                break;
            case 0x18:
                working.write(LOAD0_CR3);
                break;
            case 0x20:
                working.write(LOAD0_CR4);
                break;
            default:
                throw new IllegalStateException("Unknown Control Register Operand");
        }
    ...
    private void store0_Dd(int modrm)
    {
        switch (modrm & 0x38) {
            case 0x00:
                ...
                break;
            case 0x08:
                working.write(STORE0_DR1);
                break;
            case 0x10:
                working.write(STORE0_DR2);
                break;
            case 0x18:
                working.write(STORE0_DR3);
                break;
            case 0x30:
                working.write(STORE0_DR6);
                break;
            case 0x38:
                working.write(STORE0_DR7);
                break;
            default:
    ...
    private void load0_Dd(int modrm)
    {
        switch (modrm & 0x38) {
            case 0x00:
                ...
                break;
            case 0x08:
                working.write(LOAD0_DR1);
                break;
            case 0x10:
                working.write(LOAD0_DR2);
                break;
            case 0x18:
                working.write(LOAD0_DR3);
                break;
            case 0x30:
                working.write(LOAD0_DR6);
                break;
            case 0x38:
                working.write(LOAD0_DR7);
                break;
            default:
                throw new IllegalStateException("Unknown Debug Register Operand");
        }
    ...
    private void store1_Ew(int prefices, int modrm, int sib, int displacement)
    {
        switch (modrm & 0xc7) {
            ...
            case 0xc0:
                ...
                break;
            case 0xc1:
                working.write(STORE1_CX);
                break;
            case 0xc2:
                working.write(STORE1_DX);
                break;
            case 0xc3:
                working.write(STORE1_BX);
                break;
            case 0xc4:
                working.write(STORE1_SP);
                break;
            case 0xc5:
                working.write(STORE1_BP);
                break;
            case 0xc6:
                working.write(STORE1_SI);
                break;
            case 0xc7:
                working.write(STORE1_DI);
                break;
        }
    }
    ...
    private void store1_Ed(int prefices, int modrm, int sib, int displacement)
    {
        switch (modrm & 0xc7) {
            ...
            case 0xc0:
                ...
                break;
            case 0xc1:
                working.write(STORE1_ECX);
                break;
            case 0xc2:
                working.write(STORE1_EDX);
                break;
            case 0xc3:
                working.write(STORE1_EBX);
                break;
            case 0xc4:
                working.write(STORE1_ESP);
                break;
            case 0xc5:
                working.write(STORE1_EBP);
                break;
            case 0xc6:
                working.write(STORE1_ESI);
                break;
            case 0xc7:
                working.write(STORE1_EDI);
                break;
        }
    }
        ...
        switch (modrm & 0x38) {
            ...
            default:
                throw new IllegalStateException(
                        "Unknown DoubleWord Register Operand");
        }
    }
    ...
    private void decodeSIB(int prefices, int modrm, int sib, int displacement)
    {
        ...
        switch (sib & 0xf8) {
            ...
            case 0x40:
                working.write(ADDR_2EAX);
                break;
            case 0x48:
                working.write(ADDR_2ECX);
                break;
            case 0x50:
                working.write(ADDR_2EDX);
                break;
            case 0x58:
                working.write(ADDR_2EBX);
                break;
            case 0x60:
                break; // none
            case 0x68:
                working.write(ADDR_2EBP);
                break;
            case 0x70:
                working.write(ADDR_2ESI);
                break;
            case 0x78:
                working.write(ADDR_2EDI);
                break;

            case 0x80:
                working.write(ADDR_4EAX);
                break;
            case 0x88:
                working.write(ADDR_4ECX);
                break;
            case 0x90:
                working.write(ADDR_4EDX);
                break;
            case 0x98:
                working.write(ADDR_4EBX);
                break;
            case 0xa0:
                break; // none
            case 0xa8:
                working.write(ADDR_4EBP);
                break;
            case 0xb0:
                working.write(ADDR_4ESI);
                break;
            case 0xb8:
                working.write(ADDR_4EDI);
                break;

            case 0xc0:
                working.write(ADDR_8EAX);
                break;
            case 0xc8:
                working.write(ADDR_8ECX);
                break;
            case 0xd0:
                working.write(ADDR_8EDX);
                break;
            case 0xd8:
                working.write(ADDR_8EBX);
                break;
            case 0xe0:
                break; // none
            case 0xe8:
                working.write(ADDR_8EBP);
                break;
            case 0xf0:
                working.write(ADDR_8ESI);
                break;
            case 0xf8:
                working.write(ADDR_8EDI);
                break;
        }
    }
