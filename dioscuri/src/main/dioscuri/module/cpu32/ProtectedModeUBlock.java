...
public class ProtectedModeUBlock implements ProtectedModeCodeBlock,
    ...
    private static final ProcessorException exceptionSS = new ProcessorException(
            Processor.PROC_EXCEPTION_SS, 0, true);
    private static final ProcessorException exceptionUD = new ProcessorException(
    ...
    public InstructionSource getAsInstructionSource() {
        ...
        System
                .arraycopy(cumulativeX86Length, 0, positions, 0,
                        positions.length);
    ...
    public int execute(Processor cpu) {
        ...
        try {
            ...
            cpu.handleProtectedModeException(e.getVector(), e.hasErrorCode(), e
                    .getErrorCode());
    ...
    private final void cmpsd_a32(Segment seg0) {
        ...
        sub_o32_flags((0xffffffffl & dataOne) - (0xffffffffl & dataTwo),
                dataOne, dataTwo);
    }
    ...
    private final void repe_cmpsd_a32(Segment seg0) {
        ...
        try {
            ...
            if (used)
                sub_o32_flags(
                        (0xffffffffl & dataOne) - (0xffffffffl & dataTwo),
                        dataOne, dataTwo);
        }
    }
    ...
    private final void rep_insb_a32(int port) throws ModuleException {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setByte(addr, (byte) cpu.ioports
                            .ioPortReadByte(port));
                    count--;
                    addr -= 1;
                }
    ...
    private final void rep_insw_a32(int port) throws ModuleException {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setWord(addr, (short) cpu.ioports
                            .ioPortReadWord(port));
                    count--;
                    addr -= 2;
                }
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es
                            .setDoubleWord(addr, cpu.ioports
                                    .ioPortReadLong(port));
                    count--;
                    addr -= 4;
                }
    ...
    private final void rep_movsb_a16(Segment outSegment) {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setByte(inAddr & 0xffff, outSegment
                            .getByte(outAddr & 0xffff));
                    count--;
                    outAddr -= 1;
                    inAddr -= 1;
                }
    ...
    private final void rep_movsw_a16(Segment outSegment) {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setWord(inAddr & 0xffff, outSegment
                            .getWord(outAddr & 0xffff));
                    count--;
                    outAddr -= 2;
                    inAddr -= 2;
                }
    ...
    private final void rep_movsd_a16(Segment outSegment) {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setDoubleWord(inAddr & 0xffff, outSegment
                            .getDoubleWord(outAddr & 0xffff));
                    count--;
                    outAddr -= 4;
                    inAddr -= 4;
                }
    ...
    private final void rep_movsd_a32(Segment outSegment) {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setDoubleWord(inAddr, outSegment
    ...
    private final void rep_outsb_a16(int port, Segment storeSegment)
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.ioports.ioPortWriteByte(port, 0xffff & storeSegment
                            .getByte(addr & 0xffff));
                    count--;
                    addr -= 1;
                }
    ...
    private final void rep_outsw_a16(int port, Segment storeSegment)
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.ioports.ioPortWriteWord(port, 0xffff & storeSegment
                            .getWord(addr & 0xffff));
                    count--;
                    addr -= 2;
                }
    ...
    private final void rep_outsd_a16(int port, Segment storeSegment)
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.ioports.ioPortWriteLong(port, storeSegment
                            .getDoubleWord(addr & 0xffff));
                    count--;
                    addr -= 4;
                }
            } else {
    ...
    private final void repe_scasd_a32(int data) {
        ...
        try {
            ...
            if (used)
                ...
                sub_o32_flags((0xffffffffl & data) - (0xffffffffl & input),
                        data, input);
        }
    }
    ...
    private final void idiv_o8(byte data) {
        ...
        cpu.eax = (cpu.eax & ~0xffff) | (0xff & result)
    ...
    private final int iretToVirtual8086Mode16BitAddressing(int newCS,
        ...
        cpu.es = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord((cpu.esp + 8) & 0xffff));
        cpu.ds = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord((cpu.esp + 12) & 0xffff));
        cpu.fs = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord((cpu.esp + 16) & 0xffff));
        cpu.gs = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord((cpu.esp + 20) & 0xffff));
    ...
    private final int iret_o32_a16() {
        if (cpu.eflagsNestedTask)
            ...
            if (((tempEFlags & (1 << 17)) != 0) && (cpu.getCPL() == 0)) {
                return iretToVirtual8086Mode16BitAddressing(tempCS, tempEIP,
                        tempEFlags);
            } else {
                return iret32ProtectedMode16BitAddressing(tempCS, tempEIP,
                        tempEFlags);
            }
        }
    }
    ...
    private final int iretToVirtual8086Mode32BitAddressing(int newCS,
        ...
        cpu.es = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord(cpu.esp + 8));
        cpu.ds = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord(cpu.esp + 12));
        cpu.fs = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord(cpu.esp + 16));
        cpu.gs = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                0xffff & cpu.ss.getDoubleWord(cpu.esp + 20));
        cpu.ss = SegmentFactory.createRealModeSegment(cpu.linearMemory, newSS);
        cpu.esp = newESP;
        cpu.setCPL(3);
    ...
    private final int iret_o32_a32() {
        if (cpu.eflagsNestedTask)
            ...
            if (((tempEFlags & (1 << 17)) != 0) && (cpu.getCPL() == 0)) {
                return iretToVirtual8086Mode32BitAddressing(tempCS, tempEIP,
                        tempEFlags);
            } else {
                return iret32ProtectedMode32BitAddressing(tempCS, tempEIP,
                        tempEFlags);
            }
        }
    }
    ...
    private final int iret_o16_a16() {
        if (cpu.eflagsNestedTask)
            ...
            return iret16ProtectedMode16BitAddressing(tempCS, tempEIP,
                    tempEFlags);
        }
    }
    ...
    private final int iret_o16_a32() {
        if (cpu.eflagsNestedTask)
            ...
            return iret16ProtectedMode32BitAddressing(tempCS, tempEIP,
                    tempEFlags);
        }
    }
    ...
    private final void sysenter() {
        ...
        cpu.cs = SegmentFactory.createProtectedModeSegment(cpu.linearMemory,
                csSelector & 0xfffc, 0x00cf9b000000ffffl);
        cpu.setCPL(0);
        cpu.ss = SegmentFactory.createProtectedModeSegment(cpu.linearMemory,
                (csSelector + 8) & 0xfffc, 0x00cf93000000ffffl);
    ...
    private final void sysexit(int esp, int eip) {
        ...
        cpu.cs = SegmentFactory.createProtectedModeSegment(cpu.linearMemory,
                (csSelector + 16) | 0x3, 0x00cffb000000ffffl);
        cpu.setCPL(3);
        cpu.ss = SegmentFactory.createProtectedModeSegment(cpu.linearMemory,
                (csSelector + 24) | 0x3, 0x00cff3000000ffffl);
    ...
    private final int in_o8(int ioport) throws ModuleException {
        if (checkIOPermissionsByte(ioport))
            ...
            System.err.println("IN_O8_O16: Denied IO Port Access [port:0x"
                    + Integer.toHexString(ioport) + " cpl:" + cpu.getCPL()
                    + "]");
            throw (ProcessorException) exceptionGP;
        }
    }
    ...
    private final int in_o16(int ioport) throws ModuleException {
        if (checkIOPermissionsShort(ioport))
            ...
            System.err.println("IN_O16_O16: Denied IO Port Access [port:0x"
                    + Integer.toHexString(ioport) + " cpl:" + cpu.getCPL()
                    + "]");
            throw (ProcessorException) exceptionGP;
        }
    ...
    private final int in_o32(int ioport) throws ModuleException {
        if (checkIOPermissionsInt(ioport))
            ...
            System.err.println("IN_O32_O16: Denied IO Port Access [port:0x"
                    + Integer.toHexString(ioport) + " cpl:" + cpu.getCPL()
                    + "]");
            throw (ProcessorException) exceptionGP;
        }
    ...
    private final void out_o8(int ioport, int data) throws ModuleException {
        if (checkIOPermissionsByte(ioport))
            ...
            System.err.println("OUT_O16_O8: Denied IO Port Access [port:0x"
                    + Integer.toHexString(ioport) + " cpl:" + cpu.getCPL()
                    + "]");
            throw (ProcessorException) exceptionGP;
        }
    ...
    private final void out_o16(int ioport, int data) throws ModuleException {
        if (checkIOPermissionsShort(ioport))
            ...
            System.err.println("OUT_O16_O16: Denied IO Port Access [port:0x"
                    + Integer.toHexString(ioport) + " cpl:" + cpu.getCPL()
                    + "]");
            throw (ProcessorException) exceptionGP;
        }
    ...
    private final void out_o32(int ioport, int data) throws ModuleException {
        if (checkIOPermissionsInt(ioport))
            ...
            System.err.println("OUT_O16_O32: Denied IO Port Access [port:0x"
                    + Integer.toHexString(ioport) + " cpl:" + cpu.getCPL()
                    + "]");
            throw (ProcessorException) exceptionGP;
        }
    ...
    private final void enter_o16_a16(int frameSize, int nestingLevel) {
        ...
        if (nestingLevel > 0) {
            while (--nestingLevel != 0) {
                tempEBP -= 2;
                tempESP -= 2;
                cpu.ss.setWord(tempESP, (short) (0xFFFF & cpu.ss
                        .getWord(tempEBP)));
            }
    ...
    private final Segment ltr(int selector) {
        ...
        long descriptor = cpu.gdtr.getQuadWord(selector & 0xfff8)
                | (0x1l << 41); // set busy flag in segment descriptor
    ...
    private final void arithmetic_flags_o32(long result, int operand1,
        ...
        cpu.setAuxiliaryCarryFlag(operand1, operand2, (int) result,
                Processor.AC_XOR);
    }
    ...
    private final void add_o32_flags(long result, int operand1, int operand2) {
        ...
        cpu.setOverflowFlag((int) result, operand1, operand2,
    ...
    private final void adc_o16_flags(int result, int operand1, int operand2) {
        if (cpu.getCarryFlag() && (operand2 == 0xffff)) {
            ...
            cpu.setOverflowFlag(result, operand1, operand2,
                    Processor.OF_ADD_SHORT);
            arithmetic_flags_o16(result, operand1, operand2);
        }
    }
    ...
    private final void shr_flags(byte result, int initial, int count) {
        if (count > 0) {
            ...
            if (count == 1)
                cpu.setOverflowFlag(result, initial,
                        Processor.OF_BIT7_DIFFERENT);
    ...
    private final boolean checkIOPermissionsByte(int ioportAddress) {
        ...
        try {
            byte ioPermMapByte = cpu.tss.getByte(ioPermMapBaseAddress
                    + (ioportAddress >>> 3));
    ...
    private final boolean checkIOPermissionsShort(int ioportAddress) {
        ...
        int ioPermMapBaseAddress = 0xffff & cpu.tss.getWord(102);
        try {
            short ioPermMapShort = cpu.tss.getWord(ioPermMapBaseAddress
                    + (ioportAddress >>> 3));
