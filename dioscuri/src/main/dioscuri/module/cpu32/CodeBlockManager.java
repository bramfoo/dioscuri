...
public class CodeBlockManager {
    ...
    public CodeBlockManager(Clock clk) {
        ...
        realModeChain = new DefaultCodeBlockFactory(new RealModeUDecoder(),
                new OptimisedCompiler(clk));
        protectedModeChain = new DefaultCodeBlockFactory(
                new ProtectedModeUDecoder(), new OptimisedCompiler(clk));
        virtual8086ModeChain = new DefaultCodeBlockFactory(
                new RealModeUDecoder(), new OptimisedCompiler(clk));

        spanningRealMode = new SpanningRealModeCodeBlock(
                new CodeBlockFactory[] { realModeChain });
        spanningProtectedMode = new SpanningProtectedModeCodeBlock(
                new CodeBlockFactory[] { protectedModeChain });
        spanningVirtual8086Mode = new SpanningVirtual8086ModeCodeBlock(
                new CodeBlockFactory[] { virtual8086ModeChain });
        ...
        try {
            ...
            System.out
                    .println("Security Manager allows creation of classloader: attempting to use advanced compilers.");
            bgc = new BackgroundCompiler(new OptimisedCompiler(clk),
                    new FASTCompiler());
            compilingRealModeChain = new DefaultCodeBlockFactory(
                    new RealModeUDecoder(), bgc);
            compilingProtectedModeChain = new DefaultCodeBlockFactory(
                    new ProtectedModeUDecoder(), bgc);
        } catch (SecurityException e) {
            System.out
                    .println("Security Manager doesn't allow creation of classloader: Not using advanced compilers.");
            bgc = null;
    ...
    public RealModeCodeBlock getRealModeCodeBlockAt(Memory memory, int offset) {
        ...
        if ((block = combiner.getRealModeCodeBlockAt(memory, offset)) == null)
            if ((block = tryRealModeFactory(compilingRealModeChain, memory,
                    offset, spanningRealMode)) == null)
                if ((block = tryRealModeFactory(realModeChain, memory, offset,
                        spanningRealMode)) == null)
                    throw new IllegalStateException(
                            "Couldn't find capable block");

        ((LazyCodeBlockMemory) memory).setRealCodeBlockAt(offset
    ...
    public ProtectedModeCodeBlock getProtectedModeCodeBlockAt(Memory memory,
        ...
        if ((block = tryProtectedModeFactory(compilingProtectedModeChain,
                memory, offset, operandSizeFlag, spanningProtectedMode)) == null)
            if ((block = tryProtectedModeFactory(protectedModeChain, memory,
                    offset, operandSizeFlag, spanningProtectedMode)) == null)
                throw new IllegalStateException("Couldn't find capable block");

        ((LazyCodeBlockMemory) memory).setProtectedCodeBlockAt(offset
                & AddressSpace.BLOCK_MASK, block);
        return block;
    }
    ...
    public Virtual8086ModeCodeBlock getVirtual8086ModeCodeBlockAt(
        ...
        if ((block = tryVirtual8086ModeFactory(virtual8086ModeChain, memory,
                offset, spanningVirtual8086Mode)) == null)
            throw new IllegalStateException("Couldn't find capable block");

        ((LazyCodeBlockMemory) memory).setVirtual8086CodeBlockAt(offset
                & AddressSpace.BLOCK_MASK, block);
        return block;
    }
    public void dispose() {
