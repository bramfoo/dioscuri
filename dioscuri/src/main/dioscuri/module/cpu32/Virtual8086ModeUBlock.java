...
public class Virtual8086ModeUBlock implements Virtual8086ModeCodeBlock,
    ...
    private static final ProcessorException exceptionBR = new ProcessorException(
            Processor.PROC_EXCEPTION_BR, true);
        ...
        try {
            switch (microcodes[position++]) {
                ...
                case CLI: {
                    ...
                    else if ((cpu.getCR4() & Processor.CR4_VIRTUAL8086_MODE_EXTENSIONS) != 0)
                ...
                case STI: {
                    ...
                    else if (!cpu.eflagsVirtualInterruptPending
                            && ((cpu.getCR4() & Processor.CR4_VIRTUAL8086_MODE_EXTENSIONS) != 0))
                ...
                case HALT:
                    ...
                    // case RDTSC: long tsc = cpu.getClockCount(); reg0 = (int)tsc;
                    // reg1 = (int)(tsc >>> 32); break;
                    // case WRMSR: cpu.setMSR(reg0, (reg2 & 0xffffffffl) | ((reg1 &
                    // 0xffffffffl) << 32)); break;
                    // case RDMSR: long msr = cpu.getMSR(reg0); reg0 = (int)msr;
                    // reg1 = (int)(msr >>> 32); break;
                ...
                case SMSW:
                    ...
                    break;
                // case LMSW: cpu.setCR0((cpu.getCR0() & ~0xf) | (reg0 & 0xf));
                // break;

                // case LGDT_O16: cpu.gdtr = cpu.createDescriptorTableSegment(reg1 &
                // 0x00ffffff, reg0); break;
                // case LGDT_O32: cpu.gdtr = cpu.createDescriptorTableSegment(reg1,
                // reg0); break;
                // case LIDT_O16: cpu.idtr = cpu.createDescriptorTableSegment(reg1 &
                // 0x00ffffff, reg0); break;
                // case LIDT_O32: cpu.idtr = cpu.createDescriptorTableSegment(reg1,
                // reg0); break;
    ...
    public int execute(Processor cpu) {
        ...
        try {
            ...
            cpu.handleVirtual8086ModeException(e.getVector(), e.hasErrorCode(),
                    e.getErrorCode());
        }
    ...
    private final void enter_o16_a16(int frameSize, int nestingLevel) {
        ...
        if (nestingLevel != 0) {
            while (--nestingLevel != 0) {
                ...
                tempESP = (tempESP & ~0xffff) | ((tempESP - 2) & 0xffff);
                cpu.ss.setWord(tempESP & 0xffff, cpu.ss
                        .getWord(tempEBP & 0xffff));
            }
    ...
    private final void leave_o16_a16() {
        ...
        int tempEBP = (cpu.ebp & ~0xffff)
                | (cpu.ss.getWord(tempESP & 0xffff) & 0xffff);
    ...
    private final void pusha_a16() {
        ...
        if ((offset < 16) && ((offset & 0x1) == 0x1)) {
            if (offset < 6)
                System.err
                        .println("Should shutdown machine (PUSHA with small ESP");
            throw exceptionGP;
        }
    ...
    private final void popa_a16() {
        ...
        cpu.edi = (cpu.edi & ~0xffff)
                | (0xffff & cpu.ss.getWord(0xffff & offset));
        offset += 2;
    ...
    private final int iret_o16_a16() {
        if (cpu.eflagsIOPrivilegeLevel == 3) {
            ...
            cpu.eip = newEIP;
            cpu.cs = SegmentFactory.createRealModeSegment(cpu.linearMemory,
                    newCS);
    ...
    private final void rep_insb_a16(int port) throws ModuleException {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setByte(addr & 0xffff, (byte) cpu.ioports
                            .ioPortReadByte(port));
                    count--;
                    addr -= 1;
                }
    ...
    private final void rep_insw_a16(int port) throws ModuleException {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setWord(addr & 0xffff, (short) cpu.ioports
                            .ioPortReadWord(port));
                    count--;
                    addr -= 2;
                }
    ...
    private final void rep_insd_a16(int port) throws ModuleException {
        ...
        try {
            if (cpu.eflagsDirection) {
                while (count != 0) {
                    // check hardware interrupts
                    cpu.es.setDoubleWord(addr & 0xffff, cpu.ioports
                            .ioPortReadLong(port));
                    count--;
                    addr -= 4;
                }
    ...
    private final void scasd_a16(int data) {
        ...
        cpu.edi = (cpu.edi & ~0xffff) | (addr & 0xffff);
        // sub_o32_flags((0xffffffffl & data) - (0xffffffffl & input), data,
        // input);
