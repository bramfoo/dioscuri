...
package nl.kbna.dioscuri.module.cpu;
	...
	/**
	 * Intel opcode 86<BR>
	 * Byte-sized content exchange of memory/register (destination) and register (source).<BR>
...
public class Instruction_XCHG_EbGb implements Instruction {
	...
	public void execute()
	{
        ...
        sourceHighLow = ((addressByte & 0x20) >> 5) == 0 ? (byte) CPU.REGISTER_GENERAL_LOW : (byte) CPU.REGISTER_GENERAL_HIGH;
		sourceRegister = (cpu.decodeRegister(operandWordSize, (addressByte & 0x38) >> 3));
		
		// Execute XCHG on reg,reg or mem,reg. Determine this from mm bits of addressbyte
		if ( ((addressByte & 0xC0) >> 6) == 3)
		{
            ...
            destinationHighLow = ((addressByte & 0x04) >> 2) == 0 ? (byte) CPU.REGISTER_GENERAL_LOW : (byte) CPU.REGISTER_GENERAL_HIGH;
		...
		else
		{
			...
			// XCHG source and destination
			tempValue = cpu.getByteFromMemorySegment(addressByte, memoryReferenceLocation);
			cpu.setByteInMemorySegment(addressByte, memoryReferenceLocation, sourceRegister[sourceHighLow]);
			sourceRegister[sourceHighLow] = tempValue;
		}
	}
}
