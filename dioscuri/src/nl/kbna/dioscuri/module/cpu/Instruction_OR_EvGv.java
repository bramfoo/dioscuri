...
package nl.kbna.dioscuri.module.cpu;
	...
	/**
	 * Intel opcode 09<BR>
	 * Logical word-sized OR of memory/register (destination) and register (source).<BR>
	 * The addressbyte determines the source (rrr bits) and destination (sss bits).<BR>
	 * Flags modified: OF, SF, ZF, AF, PF, CF
	 */
public class Instruction_OR_EvGv implements Instruction {
	 ...
	 */
	public Instruction_OR_EvGv(CPU processor)
	{
		this();
	...
	public void execute()
	{
		...
		else
		{
			...
			// Test ZF on result
			cpu.flags[CPU.REGISTER_FLAGS_ZF] = logicalORResult[CPU.REGISTER_GENERAL_HIGH] == 0 && logicalORResult[CPU.REGISTER_GENERAL_LOW] == 0  ? true : false;
			// Test SF on result (set when MSB is 1, occurs when result >= 0x80)
			cpu.flags[CPU.REGISTER_FLAGS_SF] = logicalORResult[CPU.REGISTER_GENERAL_HIGH] < 0 ? true : false;
			// Set PF on lower byte of result
			cpu.flags[CPU.REGISTER_FLAGS_PF] = Util.checkParityOfByte(logicalORResult[CPU.REGISTER_GENERAL_LOW]);
