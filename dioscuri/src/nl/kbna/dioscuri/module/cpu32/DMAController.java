...
public class DMAController extends ModuleDevice implements IOPortCapable, HardwareComponent
{
    ...
    private static final int CMD_NOT_SUPPORTED = CMD_MEMORY_TO_MEMORY | CMD_FIXED_ADDRESS | CMD_COMPRESSED_TIME | CMD_CYCLIC_PRIORITY | CMD_EXTENDED_WRITE
    ...
    {
        // Redirect to native handler
        ioPortWriteLong(portAddress, ((((int) dataDoubleWord[3]) & 0xFF) << 24) + ((((int) dataDoubleWord[2]) & 0xFF) << 16)
                + ((((int) dataDoubleWord[1]) & 0xFF) << 8) + (((int) dataDoubleWord[0]) & 0xFF));
    }
