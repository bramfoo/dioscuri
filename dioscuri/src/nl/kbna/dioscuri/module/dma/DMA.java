 ...
 * ********************************************
 * general.type                : dma
 * general.name                : DMA Controller
 * general.architecture        : Von Neumann
 * general.description         : Implements a standard DMA controller
 * general.creator             : Tessella Support Services, Koninklijke Bibliotheek, Nationaal Archief of the Netherlands
 * general.version             : 1.0
 * general.keywords            : DMA, PIO, ATA, IDE, 8237
 * general.relations           : motherboard, memory
...
public class DMA extends ModuleDMA
{
    ...
    boolean terminalCountReached;       // Terminal Count; generated when transfer is complete and signals operation complete.
    byte[] ext_page_reg = new byte[16]; // Extra page registers. Can be read/written using I/O ports, but is otherwise unused.
    
    // Attributes
    // Create two DMA controllers - these are cascaded via channel 4.
    // Note on cascading: Unlike the IRQ controllers, DMA controllers are counter-intuitively cascaded so that the 
    // first ("master", or 0) has its Hold Request (HRQ) and Hold Acknowledge (HLDA) connected to the DMA Request 4 (DREQ4) 
    // and DMA Acknowledge 4 (DACK4) of the second ("slave" or 1).
    // This means that the HRQ resulting from DMA requests on channels 0 - 3 is cascaded to DREQ4, which results in an HRQ from
    // the "slave" that is passed to the CPU. So the "slave" controller actually raises the HRQ to the CPU!
    public DMAController[] controller  = new DMAController[]{new DMAController(), new DMAController()};
    
    // Create two 4-channel handlers: 8-bit (controller 0) and 16-bit (controller 1)
    ...
    // Logging
    private static Logger logger = Logger.getLogger("nl.kbna.dioscuri.module.dma");
    ...
    public DMA(Emulator owner)
    {
        ...
        for (ctrlNum = 0; ctrlNum < 2; ctrlNum++) {
          for (chanNum = 0; chanNum < 4; chanNum++) {
            controller[ctrlNum].channel[chanNum].mode.modeType = 0;             // Demand mode
            controller[ctrlNum].channel[chanNum].mode.addressDecrement = false; // Address increment
            controller[ctrlNum].channel[chanNum].mode.autoInitEnable = false;   // Autoinit disabled
            ...
            controller[ctrlNum].channel[chanNum].channelUsed = false;           // Channel not in use
          }
        }
    ...
    /**
     * Reset controller's mask, command register, status registers, and flip-flop  
     *  
    ...
    public String getDump()
    {
        ...
        for (int ctrlNum = 0; ctrlNum < 2; ctrlNum++)
        {
            ...
            {
                if (controller[ctrlNum].channel[chanNum].channelUsed) // Channel used
                {
                    if (ctrlNum == 0) // Check for 8-bit handlers
                    {
                        dump += " Channel " + chanNum + ": " + dma8Handler[chanNum].owner + "\n";
                    }
                    else
                    // Check for 16-bit handlers
                    {
                        dump += " Channel " + (chanNum + 4) + ": " + dma16Handler[chanNum].owner + "\n";
                    }
                }
            }
        }
    ...
    public byte getIOPortByte(int portAddress) throws ModuleUnknownPort
    {
        ...
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Read from port 0x" + Integer.toHexString(portAddress).toUpperCase());

        // DMA is currently only used for the floppy drive, so if no floppy I/O is used don't bother with DMA; return bogus value
        ...
        switch (portAddress)
        {
            ...
            case 0xCC:  // Controller 1, channel 3
                chanNum = (portAddress >> (1 + ctrlNum)) & 0x03;    // Determine channel number from above port addresses
            ...
            case 0xD0:  // Controller 1
                ...
                controller[ctrlNum].statusRegister &= 0xF0;     // Clear TC flags on read
                return returnValue;

            // Read controller's temporary register, used in memory-to-memory transfers
            // Unused register, so return 0 as default value
            case 0x0D:  // Controller 0
            case 0xDA:  // Controller 1
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller [" + ctrlNum + "] temporary register (unused) read. Returned 0 (default)");
                return 0;
            ...
            case 0xDE: // Controller 1
                returnValue = (byte) (controller[ctrlNum].mask[0] | 
                                     ...
                                     (controller[ctrlNum].mask[2] << 2) | 
                                     (controller[ctrlNum].mask[3] << 3));
    ...
    public void setIOPortByte(int portAddress, byte data) throws ModuleUnknownPort
    {
        ...
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " I/O write to port 0x" + Integer.toHexString(portAddress).toUpperCase() + ": 0x"
                + Integer.toHexString(data));

        // DMA is currently only used for the floppy drive, so if no floppy I/O is used don't bother with DMA; just return
        if (dma8Handler[FLOPPY_DMA_CHANNEL] == null)
            return;
        ...
        switch (portAddress)
        {
            ...
            case 0xCC:  // Controller 0, channel 3
                chanNum = (portAddress >> (1 + ctrlNum)) & 0x03;    // Determine channel number from above port addresses
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Controller " + ctrlNum + ",  channel " + chanNum + " base and current address set.");
                if (controller[ctrlNum].flipflop)
                {
                    ...
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "    base = 0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].baseAddress).toUpperCase());
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "    curr = 0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].currentAddress).toUpperCase());
                }
                else
                {
            ...
            case 0xCE:
                chanNum = (portAddress >> (1 + ctrlNum)) & 0x03;    // Determine channel number from above port addresses
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "  Controller " + ctrlNum + ",  channel " + chanNum + " base and current count set.");
                if (controller[ctrlNum].flipflop)
                {
                    // High byte; mask sign because a cast from byte occurs
                    ...
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "    base = 0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].baseCount).toUpperCase());
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + "    curr = 0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].currentCount).toUpperCase());
                }
                else
                {
                    // Lower byte; mask sign because a cast from byte occurs
            ...
            // The only supported functionality here is enable/disable controller (bit 2), report all other commands as unsupported
            case 0x08:  // Controller 0
            case 0xD0:  // Controller 1
                if ((data & 0xFB) != 0x00)  // Attempting to enter commands other than [en|dis]abling controller
                {
                    logger.log(Level.WARNING, "[" + MODULE_TYPE + "]" + " command register functionality setting not supported");
                }
                controller[ctrlNum].commandRegister = data;
                controller[ctrlNum].ctrlDisabled = ((data >> 2) & 0x01) == 1 ? true : false;
                controlHoldRequest(ctrlNum);   // Set the HoldRequest (HRQ) accordingly
                return;
            ...
            case 0xD2:  // Controller 1
                ...
                if ((data & 0x04) != 0) // Check bit 2 for set/clear
                {
                    ...
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": Set DMA request bit for channel " + chanNum);
                }
                else
                {
                    // Clear request bit
                    controller[ctrlNum].statusRegister &= ~(1 << (chanNum + 4));
                    logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": Clear DMA request bit for channel " + chanNum);
                }
                controlHoldRequest(ctrlNum);   // Set the HoldRequest (HRQ) accordingly
                return;
            ...
            case 0xD4:
                chanNum = data & 0x03;    // Determine channel number from bits 0-1
                controller[ctrlNum].mask[chanNum] = (byte) ((data & 0x04) > 0 ? 1 : 0); // Set/clear appropriate channel's mask
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ", channel " + chanNum + ": set mask as " + ((data & 0x04) > 0 ? 1 : 0) + "; mask now=0x" + controller[ctrlNum].mask[chanNum]);
                controlHoldRequest(ctrlNum);   // Set the HoldRequest (HRQ) accordingly
                return;
            ...
            case 0xD6:  // Controller 1
                ...
                controller[ctrlNum].channel[chanNum].mode.addressDecrement = ((data >> 5) & 0x01) == 1 ? true : false;
                controller[ctrlNum].channel[chanNum].mode.autoInitEnable   = ((data >> 4) & 0x01) == 1 ? true : false;
                controller[ctrlNum].channel[chanNum].mode.transferType     = (byte) ((data >> 2) & 0x03);
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ", channel " + chanNum + ": mode register set to 0x" + Integer.toHexString(data).toUpperCase());
                return;

            // Clear flip-flop
            case 0x0C:  // Controller 0
            case 0xD8:  // Controller 1
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": flip-flop cleared");
                controller[ctrlNum].flipflop = false;
                return;

            // Master clear
            // Similar to hardware reset: Command, Status, Request, and Temporary registers, and flip-flop are cleared;
            // The Mask register is set (disabling all channels)
            case 0x0D:  // Controller 0
            case 0xDA:  // Controller 1
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": master clear (reset)");
            ...
            case 0xDC:  // Controller 1
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": clear mask register");
            ...
            case 0x0F:  // Controller 0
            case 0xDE:  // Controller 1
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": write mask register");
            ...
            case 0x81:  // Controller 0, channel 2
                ...
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": page register 2 = 0x" + Integer.toHexString(data).toUpperCase());
                return;
            case 0x82:  // Controller 0, channel 3
                controller[ctrlNum].channel[3].pageRegister = data;
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": page register 3 = 0x" + Integer.toHexString(data).toUpperCase());
                return;
            case 0x83:  // Controller 0, channel 1
                controller[ctrlNum].channel[1].pageRegister = data;
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": page register 1 = 0x" + Integer.toHexString(data).toUpperCase());
                return;
            case 0x87:  // Controller 0, channel 0
                controller[ctrlNum].channel[0].pageRegister = data;
                logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " Controller " + ctrlNum + ": page register 0 = 0x" + Integer.toHexString(data).toUpperCase());
                return;
     ...
     * @throws ModuleUnknownPort
     */
    public void setIOPortWord(int portAddress, byte[] dataWord) throws ModuleUnknownPort
    {
     ...
     * @throws ModuleUnknownPort
     */
    public byte[] getIOPortDoubleWord(int portAddress) throws ModuleWriteOnlyPortException
    {
     ...
     * @throws ModuleUnknownPort
     */
    public void setIOPortDoubleWord(int portAddress, byte[] dataDoubleWord) throws ModuleUnknownPort
    {
     ...
     * @param   DMA8Handler dma8Handler The handler to register for the channel
     * 
    ...
    public boolean registerDMAChannel(int chanNum, DMA8Handler dma8handler)
    {
        if (chanNum > 3)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " registerDMA8Channel: invalid channel number: " + chanNum);
            return false; // Fail
        }
    ...
    /**
     * Allows a device to register an 16-bit DMA Handler providing implementations for the ReadFromMem and WriteToMem
     * 
     * @param int chanNum The channel to register the handler on (4 - 7)
     * @param DMA16Handler dma16Handler The handler to register for the channel
     * @return true if succesfully registered, false otherwise
     */
    public boolean registerDMAChannel(int chanNum, DMA16Handler dma16handler)
    {
        ...
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " registerDMA16Channel: invalid channel number: " + chanNum);
            return false; // Fail
        }
    ...
    public boolean unregisterDMAChannel(int chanNum)
    {
      ...
      logger.log(Level.INFO, "[" + MODULE_TYPE + "]" + " Channel " + chanNum + " no longer used");
      return true;
    }
    ...
    /**
     * Sets the DMA Requests in the corresponding controller's status register, and
     * initiates the handling of Hold Requests
     * 
     * @param chanNum       Channel requesting a DMA transfer
     * @param dmaRequest    Set request (true); clear request (false)
     */
    public void setDMARequest(int chanNum, boolean dmaRequest)
    {
        ...
        if (chanNum > 7)
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " setDMARequest(): channel " + chanNum + " not connected to any device");
        }

        ctrlNum = chanNum > 3 ? SLAVE_CTRL : MASTER_CTRL;  // Determine controller that should handle DMA request
        
        controller[ctrlNum].DRQ[chanNum & 0x03] = dmaRequest;  // Set DMA Request in the controller
        ...
        if (!dmaRequest)
        {
            ...
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " setDMARequest(): val == 0");
        ...
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " baseCount: 0x" + Integer.toHexString(controller[0].channel[chanNum].baseCount).toUpperCase());
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " currentCount: 0x" + Integer.toHexString(controller[0].channel[chanNum].currentCount).toUpperCase());
        logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " pageReg: 0x" + Integer.toHexString(controller[0].channel[chanNum].pageRegister).toUpperCase());
        ...
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " setDMARequest(): mode_type(0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].mode.modeType).toUpperCase() + " not supported");
        }
        
        // Determine the lower boundary of the DMA transfer
        dmaFloor = (controller[ctrlNum].channel[chanNum].pageRegister << 16) | (controller[ctrlNum].channel[chanNum].baseAddress << ctrlNum);
        
        // Depending on the direction of transfer in memory (up/down), determine the upper boundary of the DMA transfer
        if (controller[ctrlNum].channel[chanNum].mode.addressDecrement)
        {
            dmaCeiling = dmaFloor - (controller[ctrlNum].channel[chanNum].baseCount << ctrlNum);
        }
        else
        {
            dmaCeiling = dmaFloor + (controller[ctrlNum].channel[chanNum].baseCount << ctrlNum);
        ...
        // Ensure these boundaries are within accepted boundaries (should this be done by memory, and why these values???)
        if ((dmaFloor & (0x7fff0000 << ctrlNum)) != (dmaCeiling & (0x7fff0000 << ctrlNum)))
        {
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " dmaFloor = 0x" + Integer.toHexString(dmaFloor).toUpperCase());
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " dmaBaseCount = 0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].baseCount).toUpperCase());
            logger.log(Level.CONFIG, "[" + MODULE_TYPE + "]" + " dmaCeiling = 0x" + Integer.toHexString(dmaCeiling).toUpperCase());
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " request outside " + (64 << ctrlNum) + "k boundary");
        }
     ...
     * origin is slave), or cascaded via DREQ4 to the slave controller (if the origin is master),
     * to be eventually set/cleared depending on the DMA request status.
     *  
    ...
    private void controlHoldRequest(int ctrlNum)
    {
        ...
        {
            if (ctrlNum != 0)   // Any Hold Request from the master are cascaded via the slave
            {
                cpu.setHoldRequest(false, this);    // Request originated from slave, so clear HRQ here
            }
            else
            {
                setDMARequest(4, false);         // Request originated from master, so cascade via slave first
            }
            return;
        }
        ...
        for (chanNum = 0; chanNum < 4; chanNum++)
        {
            // Check statusRegister bits 7-4, and ensure the corresponding mask bit is clear
            if (((controller[ctrlNum].statusRegister & (1 << (chanNum + 4))) != 0) && (controller[ctrlNum].mask[chanNum] == 0))
            {
                if (ctrlNum != 0)   // Any Hold Request from the master are cascaded via the slave
                {
                    cpu.setHoldRequest(true, this);     // Request originated from slave, so set HRQ here
                }
                else
                {
                    setDMARequest(4, true);         // Request originated from master, so cascade via slave first
                }
                break;
            }
        }
    ...
    public void acknowledgeBusHold()
    {
        ...
        boolean countExpired = false;   // Extra count variable to check terminal count (TC)

        // CPU acknowledged hold, so set HLDA
        busHoldAcknowledged = true;

        // Hold is acknowledged, so act on it: select highest priority channel in slave controller
        for (chanNum = 0; chanNum < 4; chanNum++)
        {
            // Check statusRegister bits 7-4 (requests), and ensure the corresponding mask bit is clear
            if (((controller[SLAVE_CTRL].statusRegister & (1 << (chanNum + 4))) != 0) && (controller[SLAVE_CTRL].mask[chanNum] == 0))
            {
                ctrlNum = SLAVE_CTRL;
                break;
            }
        ...
        // Check cascade channel in slave to see if HRQ came from master; if so, select highest priority channel in master controller
        if (chanNum == 0)
        {
            controller[SLAVE_CTRL].DACK[0] = true;  // Acknowledge DMA Request in slave controller
            for (chanNum = 0; chanNum < 4; chanNum++)
            {
                if (((controller[MASTER_CTRL].statusRegister & (1 << (chanNum + 4))) != 0) && (controller[0].mask[chanNum] == 0))
                {
                    ctrlNum = MASTER_CTRL;
                    break;
                }
        ...
        logger.log(Level.FINE, "[" + MODULE_TYPE + "]" + " Hold ACK: OK in response to DRQ(" + chanNum + "), address 0x" + Integer.toHexString(controller[ctrlNum].channel[chanNum].currentAddress & 0xFF).toUpperCase());
        
        // Determine memory address - this is stored in the DMA channel
        memoryAddress = (  (controller[ctrlNum].channel[chanNum].pageRegister << 16) 
                         | (controller[ctrlNum].channel[chanNum].currentAddress << ctrlNum));

        logger.log(Level.FINE, "BaseAddress=" + controller[ctrlNum].channel[chanNum].baseAddress + ", currentAddress=" + controller[ctrlNum].channel[chanNum].currentAddress + ", memoryAddress=" + memoryAddress);
        ...
        // Check if word count has 'rolled' over to 0xFFFF (-1 signed); generate TC and check if autoInitialise enabled
        if (controller[ctrlNum].channel[chanNum].currentCount == -1)
        {
            // Transfer complete
            // Assert TC, deassert HRQ,  DACK(n) lines
            controller[ctrlNum].statusRegister |= (1 << chanNum); // Set TC in status register 
            ...
            {
                // AutoInit mode restores original values of currentAddress, currentCount from baseAddress, baseCount
        ...
        {
            terminalCountReached = false;       // Device has checked TC during handlerWrite()/handlerRead(), so can clear TC
            busHoldAcknowledged = false;
            ...
            controller[ctrlNum].DACK[chanNum] = false; // clear DACK to adapter card
        }
    }

    /**
     * The DMA transfer type (verify/write/read) set in the mode register is initiated
     * by calling the registered 8-bit/16-bit handler
     * 
    ...
    private void initiateDMATransfer(int ctrlNum, int chanNum, int memoryAddress) throws ModuleException
    {
        ...
        // Initiate DMA transfer; check type and channel, call registered handler to proceed
        
        if (controller[ctrlNum].channel[chanNum].mode.transferType == DMAModeRegister.DMA_TRANSFER_WRITE)  // device -> memory
        {
            ...
            else                // 16-bit transfer
            {
                if (dma16Handler[chanNum] != null)
                {
                    ...
                    logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " no dma16 write handler for channel " + chanNum);
            }
        }
        else if (controller[ctrlNum].channel[chanNum].mode.transferType == DMAModeRegister.DMA_TRANSFER_READ)   // memory -> device
        {
            if (ctrlNum == 0)   // 8-bit transfer
            {
        ...
        {
            logger.log(Level.SEVERE, "[" + MODULE_TYPE + "]" + " HLDA: memory->memory transfer (type 3) undefined");
        }
    }
    
    /**
     * Registers the cascade channel, 4.
     * Does not implement any read or write methods,<BR>
